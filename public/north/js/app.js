...
// ── BUILD 33.8 · FINANCIAL CONSISTENCY CORE ───────────
(function(){
  'use strict';
  function fx338(){var v=Number(usdBrl||0);try{if(!(v>3&&v<10))v=Number(window.nc32EffectiveFx());}catch(e){}return v>3&&v<10?v:5.08;}
  function val338(a){return Number(a.qtd||0)*pBRL(a);}
  function avgFx338(a){if(a.mercado!=='US')return 1;var v=Number(cambioMedioHistorico(a.ticker));return v>3&&v<10?v:fx338();}
  function metrics338(){var fx=fx338(),pat=0,cost=0,usValue=0,usCostUsd=0,usValueUsd=0,classes={};(carteira||[]).forEach(function(a){var v=val338(a),c=a.mercado==='US'?Number(a.qtd||0)*Number(a.pmedio||0)*avgFx338(a):Number(a.qtd||0)*Number(a.pmedio||0),k=rbClassOf(a);pat+=v;cost+=c;classes[k]=(classes[k]||0)+v;if(a.mercado==='US'){usValue+=v;usValueUsd+=Number(a.qtd||0)*Number((getP(a)||{}).preco||a.pmedio||0);usCostUsd+=Number(a.qtd||0)*Number(a.pmedio||0);}});return {fx:fx,pat:pat,cost:cost,gain:pat-cost,ret:cost?(pat-cost)/cost*100:0,usdEq:pat/fx,usValue:usValue,usValueUsd:usValueUsd,usCostUsd:usCostUsd,usGainUsd:usValueUsd-usCostUsd,usRetUsd:usCostUsd?(usValueUsd-usCostUsd)/usCostUsd*100:0,classes:classes};}
  function fmtPct338(v){return (Number(v)>=0?'+':'')+Number(v||0).toFixed(2).replace('.',',')+'%';}
  function regionValues338(){var m=metrics338(),r={Brasil:0,'Estados Unidos':0,Canadá:0,'Europa desenvolvida':0,Índia:0,'Ásia desenvolvida':0,'Outros países':0};function add(k,v){r[k]+=v;}(carteira||[]).forEach(function(a){var v=val338(a),tk=String(a.ticker||'').toUpperCase();if(a.mercado!=='US')return add('Brasil',v);if(tk==='INDA')return add('Índia',v);if(tk==='RACE')return add('Europa desenvolvida',v);if(tk==='VXUS'){add('Europa desenvolvida',v*.42);add('Ásia desenvolvida',v*.32);add('Canadá',v*.08);return add('Outros países',v*.18);}if(tk==='URA'||tk==='SHLD'){add('Estados Unidos',v*.52);add('Canadá',v*.12);add('Europa desenvolvida',v*.20);add('Ásia desenvolvida',v*.10);return add('Outros países',v*.06);}add('Estados Unidos',v);});return {total:m.pat,values:r};}
  function politicalMap338(){
    return '<svg class="nc338-political-map" viewBox="0 0 940 460" role="img" aria-label="Mapa político mundial com países expostos destacados"><g class="countries">'+
    '<path class="exposed us" d="M92 113l85-27 78 16 28 48-30 43-75 10-69-35z"><title>Estados Unidos</title></path><path class="exposed ca" d="M72 63l91-35 115 24 16 47-111-8-91 27z"><title>Canadá</title></path><path d="M137 203l52-5 33 39-31 30-37-27z"><title>México</title></path><path class="exposed br" d="M246 274l66-26 48 43-22 89-56 45-36-80z"><title>Brasil</title></path><path d="M272 382l35 33-15 39-25-31z"><title>Argentina</title></path><path d="M211 248l27 29-13 42-26-26z"><title>Colômbia</title></path><path d="M210 318l30 10-5 48-22 22z"><title>Peru</title></path><path d="M453 104l22-18 17 15-13 23z"><title>Reino Unido</title></path><path class="exposed eu" d="M475 124l34-24 27 25-16 32-38 3z"><title>França</title></path><path class="exposed eu" d="M513 95l33-5 17 34-28 18-25-21z"><title>Alemanha</title></path><path class="exposed eu" d="M529 145l23 7 12 41-22-13-17-27z"><title>Itália</title></path><path class="exposed eu" d="M450 151l35 3-12 28-38-5z"><title>Espanha</title></path><path d="M555 72l34-18 37 29-23 44-38-5z"><title>Europa Oriental</title></path><path d="M450 195l78-20 54 38-13 103-64 76-56-92z"><title>África</title></path><path d="M579 99l113-49 111 31 60 60-76 39-82-22-74 31-53-44z"><title>Rússia e Ásia Central</title></path><path class="exposed in" d="M681 214l46 16 19 58-38 54-38-72z"><title>Índia</title></path><path class="exposed asia" d="M739 167l78-20 56 48-44 67-75-26z"><title>China</title></path><path class="exposed asia" d="M858 178l22 14-8 45-18-13z"><title>Japão</title></path><path d="M748 263l42 10 23 46-38 12-30-38z"><title>Sudeste Asiático</title></path><path class="exposed asia" d="M786 345l74-20 49 45-29 55-83-12z"><title>Austrália</title></path><path d="M902 407l19 9-8 22-18-9z"><title>Nova Zelândia</title></path></g><g class="labels"><text x="160" y="148">EUA</text><text x="272" y="327">Brasil</text><text x="520" y="118">Europa</text><text x="692" y="268">Índia</text><text x="790" y="207">Ásia</text></g></svg>';
  }
  function allocBar338(label,value,total,color){var p=total?value/total*100:0;return '<div class="nc338-alloc"><div><span>'+label+'</span><b>'+p.toFixed(1).replace('.',',')+'%</b><small>'+fR(value)+'</small></div><i><b style="width:'+Math.min(100,p)+'%;background:'+color+'"></b></i></div>';}
  function heatmap338(m){var items=(carteira||[]).filter(function(a){return val338(a)>0&&a.tipo!=='caixa';}).map(function(a){var value=val338(a),score=strategyScore(a);return {a:a,value:value,weight:value/m.pat*100,score:score};}).sort(function(a,b){return b.weight-a.weight;});return '<div class="nc338-heatmap">'+items.map(function(x){var cls=x.score>=80?'excellent':x.score>=70?'good':x.score>=60?'attention':'review';return '<button class="'+cls+'" style="--size:'+Math.max(1,x.weight)+'" onclick="nc15OpenPage(\'at\')"><strong>'+esc(x.a.ticker)+'</strong><b>'+x.weight.toFixed(1)+'%</b><span>'+fR(x.value)+'</span><small>Score '+Math.round(x.score)+'</small></button>';}).join('')+'</div>';}
  function dashboard338(){var root=document.getElementById('pg-vg');if(!root)return;var m=metrics338(),rv=regionValues338(),score=qualityIndex(),last=camUpd?new Date(camUpd):new Date(),largest=(carteira||[]).filter(function(a){return val338(a)>0;}).sort(function(a,b){return val338(b)-val338(a);})[0],headerFx=document.getElementById('nc15HeaderUsd');if(headerFx)headerFx.textContent=m.fx.toFixed(4).replace('.',',');var status=document.getElementById('stlbl');if(status)status.textContent='Atualizado '+last.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});var regList=Object.keys(rv.values).filter(function(k){return rv.values[k]>0;}).map(function(k){return '<div><span>'+k+'</span><b>'+((rv.values[k]/rv.total)*100).toFixed(1).replace('.',',')+'%</b><small>'+fR(rv.values[k])+'</small></div>';}).join('');root.innerHTML='<section class="nc338-head"><div><span>EXECUTIVE CORE · BUILD 33.8</span><h1>Dashboard patrimonial</h1><p>Rentabilidade, exposição e concentração calculadas sobre a carteira real.</p></div><div class="nc338-live"><i></i><div><span>USD/BRL em tempo real</span><strong>R$ '+m.fx.toFixed(4).replace('.',',')+'</strong><small>Atualizado '+last.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})+' · '+esc(fxSource||'North Engine')+'</small></div><button onclick="refresh()">↻</button></div></section><section class="nc338-kpis"><article><span>Carteira em BRL</span><strong>'+fR(m.pat)+'</strong><small>Custo '+fR(m.cost)+'</small></article><article><span>Carteira em USD</span><strong>'+fU(m.usdEq)+'</strong><small>Equivalente pelo câmbio atual</small></article><article><span>Ganho em BRL</span><strong class="'+fC(m.gain)+'">'+fR(m.gain)+'</strong><small>'+fmtPct338(m.ret)+' sobre o custo histórico</small></article><article><span>Resultado internacional</span><strong class="'+fC(m.usGainUsd)+'">'+fU(m.usGainUsd)+'</strong><small>'+fmtPct338(m.usRetUsd)+' sem efeito cambial</small></article><article><span>Maior concentração</span><strong>'+(largest?esc(largest.ticker):'—')+'</strong><small>'+(largest?(val338(largest)/m.pat*100).toFixed(1).replace('.',',')+'% · '+fR(val338(largest)):'—')+'</small></article><article><span>North Score</span><strong>'+score+'<b>/100</b></strong><small>'+northScoreSemantic(score).label+'</small></article></section><section class="nc338-grid"><article class="nc338-panel"><header><span>ALOCAÇÃO CONSOLIDADA</span><h2>Composição atual da carteira</h2></header>'+allocBar338('Brasil',m.pat-m.usValue,m.pat,'#22d18b')+allocBar338('Exterior',m.usValue,m.pat,'#287dff')+allocBar338('Renda fixa Brasil',m.classes.BR_RENDA||0,m.pat,'#b676ef')+allocBar338('ETFs internacionais',m.classes.US_ETF||0,m.pat,'#277cff')+allocBar338('Ações internacionais',m.classes.US_ACAO||0,m.pat,'#18b978')+'</article><article class="nc338-panel nc338-map-panel"><header><span>EXPOSIÇÃO GEOGRÁFICA</span><h2>Países e regiões presentes nos ativos</h2></header><div class="nc338-map-layout">'+politicalMap338()+'<div class="nc338-region-list">'+regList+'</div></div></article></section><section class="nc338-panel nc338-heatmap-panel"><header><span>HEATMAP COMPLETO</span><h2>Todos os ativos por peso e qualidade estratégica</h2><p>O tamanho representa o peso na carteira; a cor representa o score.</p></header>'+heatmap338(m)+'</section>';}
  window.nc15BuildDashboard=dashboard338;

  function portfolio338(pat){var root=document.getElementById('tbAt');if(!root)return;var fx=fx338(),total=pat||metrics338().pat;var head=root.closest('table').querySelector('thead');head.innerHTML='<tr><th>Ativo</th><th>Qtd.</th><th>USD médio</th><th>PM compra USD</th><th>Cotação atual USD</th><th>Valor USD</th><th>Rent. USD</th><th>USD/BRL atual</th><th>Ganho BRL</th><th>Rent. BRL</th><th>Valor BRL</th><th>Peso</th><th>Ações</th></tr>';root.innerHTML=(carteira||[]).map(function(a,i){var us=a.mercado==='US',q=Number(a.qtd||0),price=Number((getP(a)||{}).preco||a.pmedio||0),avgfx=avgFx338(a),costUsd=q*Number(a.pmedio||0),valueUsd=us?q*price:val338(a)/fx,gainUsd=valueUsd-costUsd,retUsd=costUsd?gainUsd/costUsd*100:0,costBrl=us?costUsd*avgfx:q*Number(a.pmedio||0),valueBrl=val338(a),gainBrl=valueBrl-costBrl,retBrl=costBrl?gainBrl/costBrl*100:0;return '<tr><td><div class="nc338-asset"><span class="tk">'+esc(a.ticker)+'</span><strong>'+esc(a.nome||'')+'</strong><small>'+esc(a.setor||'')+'</small></div></td><td>'+q.toFixed(6)+'</td><td>'+(us?'R$ '+avgfx.toFixed(4):'—')+'</td><td>'+(us?fU(a.pmedio):fR(a.pmedio))+'</td><td>'+(us?fU(price):fR(price))+'<small class="nc338-source">'+esc((getP(a)||{}).fonte||'local')+'</small></td><td>'+(us?fU(valueUsd):'—')+'</td><td class="'+fC(retUsd)+'">'+(us?fmtPct338(retUsd):'—')+'</td><td>R$ '+fx.toFixed(4)+'</td><td class="'+fC(gainBrl)+'">'+fR(gainBrl)+'</td><td class="'+fC(retBrl)+'">'+fmtPct338(retBrl)+'</td><td><b>'+fR(valueBrl)+'</b></td><td>'+((total?valueBrl/total*100:0).toFixed(2))+'%</td><td><div class="nc338-actions"><button onclick="openM(\'edit\','+i+')">Editar</button><button class="danger" onclick="delAt('+i+')">Excluir</button></div></td></tr>';}).join('');var headBox=document.querySelector('#pg-at .nc335-portfolio-head');if(headBox&&!headBox.querySelector('.nc338-pdf-btn')){var b=document.createElement('button');b.className='bp nc338-pdf-btn';b.textContent='Baixar PDF das posições';b.onclick=window.exportPortfolioPDF;headBox.appendChild(b);}}
  buildAtivos=portfolio338;
  window.exportPortfolioPDF=function(){var m=metrics338(),rows=(carteira||[]).filter(function(a){return val338(a)>0;}).sort(function(a,b){return val338(b)-val338(a);}).map(function(a){var q=Number(a.qtd||0),price=Number((getP(a)||{}).preco||a.pmedio||0),v=val338(a),cost=a.mercado==='US'?q*Number(a.pmedio||0)*avgFx338(a):q*Number(a.pmedio||0),gain=v-cost;return '<tr><td><b>'+esc(a.ticker)+'</b><small>'+esc(a.nome||'')+'</small></td><td>'+esc(a.mercado)+'</td><td>'+q.toFixed(6)+'</td><td>'+(a.mercado==='US'?'R$ '+avgFx338(a).toFixed(4):'—')+'</td><td>'+(a.mercado==='US'?fU(a.pmedio):fR(a.pmedio))+'</td><td>'+(a.mercado==='US'?fU(price):fR(price))+'</td><td>'+fR(v)+'</td><td class="'+fC(gain)+'">'+fR(gain)+'</td><td>'+fmtPct338(cost?gain/cost*100:0)+'</td><td>'+(v/m.pat*100).toFixed(2)+'%</td></tr>';}).join('');printReport338('Posições da carteira','<div class="kpis"><div>Patrimônio BRL<b>'+fR(m.pat)+'</b></div><div>Patrimônio USD<b>'+fU(m.usdEq)+'</b></div><div>Ganho BRL<b>'+fR(m.gain)+'</b></div><div>USD/BRL<b>'+m.fx.toFixed(4)+'</b></div></div><table><thead><tr><th>Ativo</th><th>Mercado</th><th>Qtd.</th><th>FX médio</th><th>PM</th><th>Cotação</th><th>Valor BRL</th><th>Ganho BRL</th><th>Rent. BRL</th><th>Peso</th></tr></thead><tbody>'+rows+'</tbody></table>');};
  function printReport338(title,body){var html='<!doctype html><html><head><meta charset="utf-8"><title>'+title+'</title><style>@page{size:A4 landscape;margin:10mm}body{font:10px Arial;color:#14202b;margin:24px}header{display:flex;justify-content:space-between;border-bottom:2px solid #b78a33;padding-bottom:12px}h1{font:22px Georgia;margin:0}.gold{color:#9b7226}.kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:15px 0}.kpis div{border:1px solid #d8dde3;padding:9px}.kpis b{display:block;font-size:13px;margin-top:4px}table{width:100%;border-collapse:collapse}th,td{padding:6px 4px;border-bottom:1px solid #d8dde3;text-align:left}th{font-size:8px;text-transform:uppercase;color:#647383}td small{display:block;color:#687786}</style></head><body><header><div><h1>NORTH CAPITAL</h1><span class="gold">PRIVATE WEALTH INTELLIGENCE</span></div><div>'+title+'<br>'+new Date().toLocaleString('pt-BR')+'</div></header>'+body+'<script>window.onload=function(){window.print()}<\/script></body></html>';var w=window.open('','_blank');if(!w){toast('Permita pop-ups para gerar o PDF');return;}w.document.write(html);w.document.close();}

  var reb338=buildRebalance;buildRebalance=function(){reb338();var p=window.__rebalancePlan;if(!p)return;var fx=Number(p.fx)||fx338(),base=metrics338().pat;p.items.sort(function(a,b){return b.amount-a.amount;});p.items.forEach(function(x){x.currentPct=base?val338(x.asset)/base*100:0;x.afterPct=(base+p.aporte)?(val338(x.asset)+x.amount)/(base+p.aporte)*100:0;x.amountUsd=x.asset.mercado==='US'?x.amount/fx:0;});var tb=document.getElementById('tbReb');if(tb){var table=tb.closest('table'),th=table.querySelector('thead');th.innerHTML='<tr><th>#</th><th>Ativo</th><th>Mercado</th><th>Aporte USD</th><th>Aporte BRL</th><th>% atual</th><th>% depois</th><th>Qtd.</th><th>Ação</th></tr>';tb.innerHTML=p.items.map(function(x,i){return '<tr><td>'+(i+1)+'</td><td><span class="tk">'+esc(x.asset.ticker)+'</span><small>'+esc(x.asset.nome||'')+'</small></td><td>'+esc(x.asset.mercado)+'</td><td>'+(x.asset.mercado==='US'?fU(x.amountUsd):'—')+'</td><td><b>'+fR(x.amount)+'</b></td><td>'+x.currentPct.toFixed(2)+'%</td><td>'+x.afterPct.toFixed(2)+'%</td><td>'+Number(x.qty||0).toFixed(6)+'</td><td><span class="reb-buy">COMPRAR</span></td></tr>';}).join('');}};
  window.exportRebalancePDF=function(){var p=window.__rebalancePlan;if(!p||!p.aporte){toast('Informe um aporte antes de gerar o PDF');return;}var fx=Number(p.fx)||fx338(),rows=p.items.slice().sort(function(a,b){return b.amount-a.amount;}).map(function(x,i){return '<tr><td>'+(i+1)+'</td><td><b>'+esc(x.asset.ticker)+'</b><small>'+esc(x.asset.nome||'')+'</small></td><td>'+esc(x.asset.mercado)+'</td><td>'+(x.asset.mercado==='US'?fU(x.amount/fx):'—')+'</td><td>'+fR(x.amount)+'</td><td>'+Number(x.currentPct||0).toFixed(2)+'%</td><td>'+Number(x.afterPct||0).toFixed(2)+'%</td><td>'+Number(x.qty||0).toFixed(6)+'</td></tr>';}).join('');printReport338('Simulação de rebalanceamento','<div class="kpis"><div>Aporte total<b>'+fR(p.aporte)+'</b></div><div>USD/BRL simulado<b>'+fx.toFixed(4)+'</b></div><div>Ativos contemplados<b>'+p.items.length+'</b></div><div>Valor não alocado<b>'+fR((p.sobra||0)+(Number(p.pending)||0))+'</b></div></div><table><thead><tr><th>#</th><th>Ativo</th><th>Mercado</th><th>Aporte USD</th><th>Aporte BRL</th><th>% atual</th><th>% depois</th><th>Qtd.</th></tr></thead><tbody>'+rows+'</tbody></table>');};

  function intelligence338(){try{buildNorthReferenceIntelligence();}catch(e){}var heat=document.querySelector('#pg-fi .nc-heatmap-panel');if(heat)heat.style.display='none';var events=document.querySelector('#pg-fi .nc-bottom-grid');if(events)events.style.display='none';var gauge=document.querySelector('#pg-fi .nc-gauge-panel');if(gauge){var needle=document.getElementById('ncGaugeNeedle');if(needle)needle.style.display='none';var svg=gauge.querySelector('svg'),score=qualityIndex(),angle=Math.PI-(score/100)*Math.PI,cx=230+175*Math.cos(angle),cy=225-175*Math.sin(angle),dot=document.getElementById('nc338GaugeDot');if(!dot&&svg){dot=document.createElementNS('http://www.w3.org/2000/svg','circle');dot.id='nc338GaugeDot';dot.setAttribute('r','10');dot.setAttribute('class','nc338-gauge-dot');svg.appendChild(dot);}if(dot){dot.setAttribute('cx',cx.toFixed(1));dot.setAttribute('cy',cy.toFixed(1));}}}
  function editDividend338(index){var d=dividendos[index];if(!d)return;var value=prompt('Valor bruto:',String(d.valor||0));if(value===null)return;var tax=prompt('Imposto:',String(d.imposto||0));if(tax===null)return;var date=prompt('Data (dd/mm/aaaa):',d.data||'');if(date===null||!validDate(date)){toast('Data inválida');return;}var period=prompt('Periodicidade:',d.periodo||'trimestral');if(period===null)return;var obs=prompt('Observação:',d.obs||'');d.valor=Math.max(0,Number(String(value).replace(',','.'))||0);d.imposto=Math.max(0,Number(String(tax).replace(',','.'))||0);d.data=date;d.periodo=period;d.obs=obs||'';save();buildDividendos();toast('Dividendo atualizado');}
  function deleteDividend338(index){var d=dividendos[index];if(!d)return;if(!confirm('Excluir o dividendo de '+d.ticker+' em '+d.data+'?'+(d.reinvestidoEm?' O reinvestimento já registrado será preservado.':'')))return;dividendos.splice(index,1);save();buildDividendos();toast('Dividendo excluído');}
  window.editDividend338=editDividend338;window.deleteDividend338=deleteDividend338;
  var div338=buildDividendos;buildDividendos=function(){div338();var cards=document.querySelectorAll('#dvHistorico .nc337-dividend');cards.forEach(function(card,reverseIndex){var index=dividendos.length-1-reverseIndex,bar=card.querySelector('.nc337-dividend-main>div:last-child');if(bar&&!bar.querySelector('.nc338-div-actions')){var actions=document.createElement('div');actions.className='nc338-div-actions';actions.innerHTML='<button onclick="editDividend338('+index+')">Editar</button><button class="danger" onclick="deleteDividend338('+index+')">Excluir</button>';bar.appendChild(actions);}});};
  var page338=nc15OpenPage;nc15OpenPage=function(id,button){page338(id,button);document.body.classList.toggle('nc338-portfolio-mode',id==='at');if(id==='vg')setTimeout(dashboard338,0);if(id==='fi')setTimeout(intelligence338,0);if(id==='dv')setTimeout(buildDividendos,0);if(id==='at')setTimeout(function(){portfolio338(metrics338().pat);},0);};
  window.nc338Dashboard=dashboard338;window.nc338Portfolio=portfolio338;window.nc338Rebalance=buildRebalance;window.nc338RebalancePDF=exportRebalancePDF;window.nc338DividendBuild=buildDividendos;window.nc338OpenPage=nc15OpenPage;
  document.addEventListener('DOMContentLoaded',function(){document.body.classList.add('nc338');setTimeout(function(){dashboard338();var h=document.querySelector('.nc15-market-status');if(h){var st=document.getElementById('stlbl');if(st)st.textContent='USD/BRL atualizado';}},1100);});
})();

// ── BUILD 33.6 · EXECUTIVE DATA CORRECTIONS ───────────
(function(){
  'use strict';
  document.body.classList.add('nc336');
  function effectiveFx(){var v=Number(usdBrl||0);if(v>3&&v<10)return v;try{v=Number(window.nc32EffectiveFx());}catch(e){}return v>3&&v<10?v:5.08;}
  function seedFx(){if(!(Number(usdBrl)>3&&Number(usdBrl)<10)){usdBrl=effectiveFx();fxSource='último câmbio válido';fxIsFresh=false;}}
  seedFx();

  // Same-origin market endpoint avoids browser CORS failures and keeps the local cache as fallback.
  var legacyFetchAll336=fetchAll;
  fetchAll=async function(){
    var tradable=(carteira||[]).filter(function(a){return a.mercado==='US'&&a.tipo!=='caixa';});
    try{
      var symbols=tradable.map(function(a){return a.ticker;}).join(',');
      var response=await fetch('/api/market?symbols='+encodeURIComponent(symbols),{cache:'no-store'});
      if(!response.ok)throw new Error('market '+response.status);
      var data=await response.json(),updated=0;
      if(Number(data.fx)>3&&Number(data.fx)<10){usdBrl=Number(data.fx);fxSource='Frankfurter via North Engine';fxIsFresh=true;camUpd=data.updatedAt||new Date().toISOString();}
      tradable.forEach(function(a){var q=data.quotes&&data.quotes[a.ticker];if(q&&Number(q.price)>0){precos[a.ticker]={preco:Number(q.price),var:0,moeda:'USD',semDados:false,fonte:q.source||'North Engine',ts:Date.now()};updated++;}});
      (carteira||[]).filter(function(a){return a.mercado!=='US'||a.tipo==='caixa';}).forEach(function(a){if(!precos[a.ticker])precos[a.ticker]={preco:Number(a.pmedio)||1,var:0,moeda:a.mercado==='US'?'USD':'BRL',semDados:false,fonte:'LOCAL',ts:Date.now()};});
      seedFx();saveQuoteCache();return {updated:updated,total:tradable.length,fxOk:Number(data.fx)>3};
    }catch(error){console.warn('North Engine proxy unavailable; preserving cached data',error);seedFx();return legacyFetchAll336();}
  };

  // Prevent zeroing international positions whenever an online provider is unavailable.
  pBRL=function(a){var fx=effectiveFx(),p=getP(a),price=Number(p&&p.preco||a.pmedio||0);if(a.tipo==='caixa')return a.mercado==='US'?fx:1;return a.mercado==='US'?price*fx:price;};
  pmBRL=function(a){return Number(a.pmedio||0)*(a.mercado==='US'?effectiveFx():1);};

  function largestPosition(pat){return (carteira||[]).filter(function(a){return rbValue(a)>0;}).map(function(a){return {a:a,v:rbValue(a)};}).sort(function(a,b){return b.v-a.v;})[0]||null;}
  function buildDecisionDashboard(){
    seedFx();var t=totais(),pat=t.pat||1,us=(carteira||[]).filter(function(a){return a.mercado==='US';}).reduce(function(s,a){return s+rbValue(a);},0),largest=largestPosition(pat),cash=cashUSDAsset(),cashUsd=cash?Number(cash.qtd||0):0;
    var set=function(id,value){var e=document.getElementById(id);if(e)e.textContent=value;};
    set('nc15Patrimonio',fR(t.pat));set('nc15TodayPct',fR(t.custo));set('nc15SincePct',fR(t.res)+' · '+fP(t.rp));
    set('nc15DailyValue',(us/pat*100).toFixed(1).replace('.',',')+'%');set('nc15DailyPct',fR(us)+' no exterior');
    set('nc15AccumValue',largest?largest.a.ticker:'—');set('nc15AccumPct',largest?(largest.v/pat*100).toFixed(1).replace('.',',')+'% do patrimônio':'—');
    set('nc15Dollar','R$ '+effectiveFx().toFixed(4).replace('.',','));set('nc15HeaderUsd',effectiveFx().toFixed(4).replace('.',','));set('nc15RibbonUsd',effectiveFx().toFixed(4).replace('.',','));
    set('nc15DollarChange',fxIsFresh?'cotação atual · '+fxSource:'último câmbio válido · '+fxSource);set('nc15CashTotal',fU(cashUsd));set('nc15CashTotalBrl',fR(cashUsd*effectiveFx())+' convertido');
    var wealth=document.getElementById('nc15WealthSpark');if(wealth)wealth.innerHTML='<div class="nc336-decision-bars"><span>Capital investido <b>'+fR(t.custo)+'</b></span><i><b style="width:'+Math.min(100,t.custo/pat*100)+'%"></b></i><span>Resultado acumulado <b class="'+fC(t.res)+'">'+fR(t.res)+'</b></span></div>';
    var perf=document.getElementById('nc15PerfSpark');if(perf)perf.innerHTML='<div class="nc336-decision-bars"><span>Brasil <b>'+((pat-us)/pat*100).toFixed(1)+'%</b></span><i><b style="width:'+((pat-us)/pat*100)+'%"></b></i><span>Exterior <b>'+((us/pat)*100).toFixed(1)+'%</b></span></div>';
  }
  var legacyDashboard336=nc15BuildDashboard;nc15BuildDashboard=function(){legacyDashboard336();buildDecisionDashboard();buildIntelligenceGeo();};

  function geoRows(){var pat=totais().pat||1,r={Brasil:0,'América do Norte':0,Europa:0,'Ásia':0,'Global / Outros':0};function add(k,v){r[k]+=Number(v||0);} (carteira||[]).forEach(function(a){var v=rbValue(a),tk=String(a.ticker||'').toUpperCase(),s=String(a.setor||'').toLowerCase();if(a.mercado!=='US')return add('Brasil',v);if(tk==='VXUS'){add('Europa',v*.42);add('Ásia',v*.38);return add('Global / Outros',v*.20);}if(tk==='INDA'||s.indexOf('india')>=0)return add('Ásia',v);if(tk==='RACE')return add('Europa',v);if(tk==='URA'||tk==='SHLD'){add('América do Norte',v*.55);add('Europa',v*.20);add('Ásia',v*.15);return add('Global / Outros',v*.10);}add('América do Norte',v);});return Object.keys(r).map(function(k){return {name:k,value:r[k],pct:r[k]/pat*100};}).sort(function(a,b){return b.value-a.value;});}
  window.buildIntelligenceGeo=function(){var root=document.getElementById('fiGeoExecutive');if(!root)return;var colors=['#22d18b','#2d82ff','#4dd29a','#f2bd22','#8599ad'];root.innerHTML=geoRows().map(function(x,i){return '<article class="nc336-geo-card" style="--geo:'+colors[i]+'" onclick="nc15OpenPage(\'at\');nc335FilterPortfolio(\''+x.name+'\')"><span>'+esc(x.name)+'</span><strong>'+x.pct.toFixed(1).replace('.',',')+'%</strong><small>'+fR(x.value)+'</small></article>';}).join('');};

  // Contribution FX only affects the number of USD units purchased, not the valuation of current positions.
  var legacyRebalance336=buildRebalance;
  buildRebalance=function(){
    legacyRebalance336();var plan=window.__rebalancePlan;if(!plan)return;var fxInput=document.getElementById('rbCambio'),fx=Number(fxInput&&fxInput.value)||effectiveFx();if(fxInput&&!fxInput.value)fxInput.value=fx.toFixed(4);
    plan.fx=fx;plan.items.forEach(function(item){if(item.asset.mercado==='US'){if(item.asset.tipo==='caixa')item.qty=item.amount/fx;else{var px=Number((getP(item.asset)||{}).preco||item.asset.pmedio||0);item.qty=px?item.amount/fx/px:0;}}else{var brpx=pBRL(item.asset);item.qty=brpx?item.amount/brpx:0;}});
    document.querySelectorAll('#tbReb tr').forEach(function(row){var tk=row.querySelector('.tk');if(!tk)return;var item=plan.items.find(function(x){return x.asset.ticker===tk.textContent.trim();});if(!item)return;var cells=row.querySelectorAll('td');if(cells[8])cells[8].innerHTML=item.asset.mercado==='US'?'<b>'+fU(item.amount/fx)+'</b><small class="nc336-br-ref">'+fR(item.amount)+'</small>':fR(item.amount);if(cells[9])cells[9].textContent=item.qty.toFixed(6);});
    var note=document.getElementById('rbResumo');if(note){var existing=note.querySelector('.nc336-rb-fx');if(existing)existing.remove();var n=document.createElement('div');n.className='nc336-rb-fx';n.innerHTML='Conversão do aporte internacional: <b>USD/BRL '+fx.toFixed(4)+'</b>';note.prepend(n);}
  };

  window.exportRebalancePDF=function(){
    var p=window.__rebalancePlan;if(!p||!p.aporte){toast('Informe um aporte antes de gerar o PDF');return;}var fx=Number(p.fx)||effectiveFx(),pat=totais().pat;
    var positions=(carteira||[]).filter(function(a){return rbValue(a)>0;}).sort(function(a,b){return rbValue(b)-rbValue(a);}).map(function(a){var v=rbValue(a);return '<tr><td><b>'+esc(a.ticker)+'</b><small>'+esc(a.nome||'')+'</small></td><td>'+esc(a.mercado)+'</td><td>'+Number(a.qtd||0).toFixed(6)+'</td><td>'+(a.mercado==='US'?fU(Number((getP(a)||{}).preco||a.pmedio||0)):fR(Number((getP(a)||{}).preco||a.pmedio||0)))+'</td><td>'+fR(v)+'</td><td>'+(v/pat*100).toFixed(2)+'%</td></tr>';}).join('');
    var buys=p.items.map(function(x,i){return '<tr><td>'+(i+1)+'</td><td><b>'+esc(x.asset.ticker)+'</b><small>'+esc(x.asset.nome||'')+'</small></td><td>'+esc(rbClassLabel(rbClassOf(x.asset)))+'</td><td>'+(x.asset.mercado==='US'?fU(x.amount/fx)+'<small>'+fR(x.amount)+'</small>':fR(x.amount))+'</td><td>'+Number(x.qty||0).toFixed(6)+'</td></tr>';}).join('');
    var html='<!doctype html><html><head><meta charset="utf-8"><title>North Capital — Posições e Aporte</title><style>@page{size:A4 landscape;margin:11mm}body{font:11px Arial;color:#14202b;margin:28px}header{display:flex;justify-content:space-between;border-bottom:2px solid #b78a33;padding-bottom:13px}h1{font:24px Georgia;margin:0}.gold{color:#9b7226;font-size:12px}h2{font-size:14px;margin:22px 0 8px}table{width:100%;border-collapse:collapse}th,td{border-bottom:1px solid #d8dde3;text-align:left;padding:7px 5px}th{font-size:8px;text-transform:uppercase;color:#647383}td small{display:block;color:#687786;margin-top:2px}.kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:16px 0}.kpis div{border:1px solid #d8dde3;padding:10px}.kpis span{display:block;color:#647383;font-size:8px;text-transform:uppercase}.kpis b{font-size:14px}footer{margin-top:20px;color:#687786;font-size:8px}</style></head><body><header><div><h1>NORTH CAPITAL</h1><span class="gold">PRIVATE WEALTH INTELLIGENCE</span></div><div>EXECUTIVE CORE · '+new Date().toLocaleString('pt-BR')+'</div></header><div class="kpis"><div><span>Patrimônio</span><b>'+fR(pat)+'</b></div><div><span>Novo aporte</span><b>'+fR(p.aporte)+'</b></div><div><span>USD/BRL do aporte</span><b>'+fx.toFixed(4)+'</b></div><div><span>Ativos sugeridos</span><b>'+p.items.length+'</b></div></div><h2>Posições atuais</h2><table><thead><tr><th>Ativo</th><th>Mercado</th><th>Quantidade</th><th>Cotação</th><th>Valor BRL</th><th>Peso</th></tr></thead><tbody>'+positions+'</tbody></table><h2>Plano do novo aporte</h2><table><thead><tr><th>#</th><th>Ativo</th><th>Classe</th><th>Valor do aporte</th><th>Quantidade estimada</th></tr></thead><tbody>'+buys+'</tbody></table><footer>Documento gerado localmente. Confira cotações e quantidades antes da execução.</footer><script>window.onload=function(){window.print()}<\/script></body></html>';var w=window.open('','_blank');if(!w){toast('Permita pop-ups para gerar o PDF');return;}w.document.write(html);w.document.close();
  };

  var legacyOpenPage336=nc15OpenPage;nc15OpenPage=function(id,button){legacyOpenPage336(id,button);document.body.classList.toggle('nc-fi-mode',id==='fi');if(id==='fi')setTimeout(function(){buildIntelligenceGeo();nc15BuildGeo();},0);};
  window.nc336FetchAll=fetchAll;window.nc336BuildRebalance=buildRebalance;window.nc336ExportRebalancePDF=exportRebalancePDF;window.nc336BuildDashboard=nc15BuildDashboard;window.nc336OpenPage=nc15OpenPage;
  document.addEventListener('DOMContentLoaded',function(){document.body.classList.add('nc336');seedFx();var rb=document.getElementById('rbCambio');if(rb&&!rb.value)rb.value=effectiveFx().toFixed(4);setTimeout(function(){try{renderAll();buildDecisionDashboard();buildIntelligenceGeo();}catch(e){console.error('Build 33.6',e);}},900);});
})();
...
(function(){
  ...
  if(window.nc336FetchAll)fetchAll=window.nc336FetchAll;
  if(window.nc336BuildRebalance)buildRebalance=window.nc336BuildRebalance;
  if(window.nc336ExportRebalancePDF)exportRebalancePDF=window.nc336ExportRebalancePDF;
  if(window.nc336BuildDashboard)nc15BuildDashboard=window.nc336BuildDashboard;
  if(window.nc336OpenPage)nc15OpenPage=window.nc336OpenPage;
...
// ── BUILD 33.7 · DECISION DASHBOARD & DIVIDEND LEDGER ──
(function(){
  'use strict';
  function fx337(){var v=Number(usdBrl||0);try{if(!(v>3&&v<10))v=Number(window.nc32EffectiveFx());}catch(e){}return v>3&&v<10?v:5.08;}
  function money(v){return fR(Number(v)||0);}
  function percent(v){return (Number(v)||0).toFixed(1).replace('.',',')+'%';}
  function currentValue(a){return Number(a.qtd||0)*pBRL(a);}
  function allMetrics(){
    var t=totais(),pat=t.pat||1,us=0,br=0,cashBR=0,cashUSD=0,classes={};
    (carteira||[]).forEach(function(a){var v=currentValue(a),k=rbClassOf(a);classes[k]=(classes[k]||0)+v;if(a.mercado==='US')us+=v;else br+=v;if(a.tipo==='caixa'&&a.mercado==='US')cashUSD+=Number(a.qtd||0);if(a.tipo==='caixa'&&a.mercado==='BR')cashBR+=v;});
    var positions=(carteira||[]).filter(function(a){return currentValue(a)>0;}).sort(function(a,b){return currentValue(b)-currentValue(a);});
    return {t:t,pat:pat,us:us,br:br,cashBR:cashBR,cashUSD:cashUSD,classes:classes,positions:positions};
  }
  function regions337(){var m=allMetrics(),r={Brasil:0,'América do Norte':0,Europa:0,'Ásia':0,'Global / Outros':0};function add(k,v){r[k]+=v;}(carteira||[]).forEach(function(a){var v=currentValue(a),tk=String(a.ticker||'').toUpperCase(),s=String(a.setor||'').toLowerCase();if(a.mercado!=='US')return add('Brasil',v);if(tk==='VXUS'){add('Europa',v*.42);add('Ásia',v*.38);return add('Global / Outros',v*.20);}if(tk==='INDA'||s.indexOf('india')>=0)return add('Ásia',v);if(tk==='RACE')return add('Europa',v);if(tk==='URA'||tk==='SHLD'){add('América do Norte',v*.55);add('Europa',v*.20);add('Ásia',v*.15);return add('Global / Outros',v*.10);}add('América do Norte',v);});return Object.keys(r).map(function(name){return{name:name,value:r[name],pct:r[name]/m.pat*100};});}
  function mapSvg337(){return '<svg class="nc337-map-svg" viewBox="0 0 760 360" role="img" aria-label="Mapa mundial da exposição geográfica"><path class="na" d="M70 83l31-24 45-7 34 15 33-7 25 17-13 21-30 7-14 23-25 4-8 25-22 7-18-20-28-5-13-24-24-10 10-23z"/><path class="sa" d="M188 166l29 8 24 28 7 32-13 27-9 42-23 28-17-26 3-31-16-23 8-29-13-27z"/><path class="eu" d="M327 77l23-20 31 4 18 15 23-5 21 14-7 18-25 1-18 17-25-6-13 13-28-5-14-19z"/><path class="af" d="M345 133l45-13 42 20 19 42-10 45-28 48-30 13-21-34-11-39-22-26 5-34z"/><path class="as" d="M437 76l39-26 64-9 55 13 51-3 48 24 29 30-18 30-42 5-33 22-45-7-32 23-34-12-27-30-35-7-16-26z"/><path class="oc" d="M618 239l38-12 42 17 17 29-23 24-44-7-29-23z"/></svg>';}
  function bar337(label,value,total,color,meta){var pct=total?value/total*100:0;return '<div class="nc337-bar"><div><span>'+label+'</span><strong>'+percent(pct)+'</strong></div><i><b style="width:'+Math.min(100,pct)+'%;background:'+color+'"></b></i><small>'+money(value)+(meta!=null?' · meta '+Number(meta).toFixed(0)+'%':'')+'</small></div>';}
  function buildDecisionDashboard337(){
    var page=document.getElementById('pg-vg');if(!page)return;var m=allMetrics(),largest=m.positions[0],score=qualityIndex(),regs=regions337(),meta=rbClassMeta(),fx=fx337();
    var top=m.positions.slice(0,6).map(function(a,i){var v=currentValue(a);return '<div class="nc337-position"><b>'+(i+1)+'</b><span><strong>'+esc(a.ticker)+'</strong><small>'+esc(a.nome||'')+'</small></span><em>'+money(v)+'<small>'+percent(v/m.pat*100)+'</small></em></div>';}).join('');
    var gaps=Object.keys(meta).map(function(k){var actual=(m.classes[k]||0)/m.pat*100;return {key:k,actual:actual,target:meta[k]||0,gap:actual-(meta[k]||0)};}).sort(function(a,b){return Math.abs(b.gap)-Math.abs(a.gap);}).slice(0,6).map(function(x){return '<div class="nc337-gap"><span>'+esc(rbClassLabel(x.key))+'</span><b>'+percent(x.actual)+'</b><em>meta '+percent(x.target)+'</em><strong class="'+(Math.abs(x.gap)<2?'ok':x.gap>0?'over':'under')+'">'+(x.gap>0?'+':'')+x.gap.toFixed(1)+' p.p.</strong></div>';}).join('');
    var regionCards=regs.map(function(x){return '<button onclick="nc15OpenPage(\'at\');nc335FilterPortfolio(\''+x.name+'\')"><span>'+esc(x.name)+'</span><strong>'+percent(x.pct)+'</strong><small>'+money(x.value)+'</small></button>';}).join('');
    page.innerHTML='<section class="nc337-dash-head"><div><span>EXECUTIVE CORE · VISÃO PATRIMONIAL</span><h1>Dashboard da carteira</h1><p>O que importa para decidir o próximo aporte e controlar os riscos.</p></div><div class="nc337-update"><i></i><span id="stlbl337">Dados consolidados</span><button onclick="refresh()">↻ Atualizar</button></div></section>'+
      '<section class="nc337-kpis"><article><span>Patrimônio</span><strong>'+money(m.t.pat)+'</strong><small>Investido '+money(m.t.custo)+'</small></article><article><span>Resultado acumulado</span><strong class="'+fC(m.t.res)+'">'+money(m.t.res)+'</strong><small>'+fP(m.t.rp)+' sobre o capital</small></article><article><span>Exterior</span><strong>'+percent(m.us/m.pat*100)+'</strong><small>'+money(m.us)+' · meta '+policy.global.US+'%</small></article><article><span>Maior posição</span><strong>'+(largest?esc(largest.ticker):'—')+'</strong><small>'+(largest?percent(currentValue(largest)/m.pat*100)+' · '+money(currentValue(largest)):'sem posições')+'</small></article><article><span>Liquidez</span><strong>'+money(m.cashBR+m.cashUSD*fx)+'</strong><small>'+fU(m.cashUSD)+' em caixa USD</small></article><article><span>North Score</span><strong>'+score+'<b>/100</b></strong><small>'+northScoreSemantic(score).label+'</small></article></section>'+
      '<section class="nc337-main-grid"><article class="nc337-panel nc337-allocation"><header><div><span>ALOCação consolidada</span><h2>Onde está o patrimônio</h2></div></header><div class="nc337-allocation-cols"><div>'+bar337('Brasil',m.br,m.pat,'#22d18b',policy.global.BR)+bar337('Exterior',m.us,m.pat,'#287dff',policy.global.US)+'</div><div>'+bar337('Renda fixa BR',m.classes.BR_RENDA||0,m.pat,'#c084fc')+bar337('ETFs internacionais',m.classes.US_ETF||0,m.pat,'#287dff')+bar337('Ações internacionais',m.classes.US_ACAO||0,m.pat,'#22c47c')+bar337('Caixa total',m.cashBR+m.cashUSD*fx,m.pat,'#f0ae22')+'</div></div></article><article class="nc337-panel"><header><div><span>CONCENTRAÇÃO</span><h2>Maiores posições</h2></div><button onclick="nc15OpenPage(\'at\')">Ver portfólio</button></header><div class="nc337-positions">'+top+'</div></article></section>'+
      '<section class="nc337-main-grid"><article class="nc337-panel nc337-geo"><header><div><span>EXPOSIÇÃO GEOGRÁFICA</span><h2>Distribuição regional funcional</h2></div></header><div class="nc337-geo-body">'+mapSvg337()+'<div class="nc337-region-cards">'+regionCards+'</div></div></article><article class="nc337-panel"><header><div><span>DESVIOS DA POLÍTICA</span><h2>Onde o próximo aporte faz diferença</h2></div><button onclick="nc15OpenPage(\'rb\')">Simular aporte</button></header><div class="nc337-gaps">'+gaps+'</div></article></section>';
  }
  window.nc15BuildDashboard=buildDecisionDashboard337;

  // Portfolio is a working ledger: controls + positions, without duplicated dashboard summaries.
  function simplifyPortfolio337(){var head=document.querySelector('#pg-at .nc335-portfolio-head');if(!head)return;var title=head.querySelector('h2'),copy=head.querySelector('p'),tag=head.querySelector('span');if(title)title.textContent='Posições da carteira';if(copy)copy.textContent='Tabela operacional com quantidades, custos, câmbio, cotações e pesos.';if(tag)tag.textContent='PORTFÓLIO';}

  function dividendNet337(d){return Math.max(0,Number(d.valor||0)-Number(d.imposto||0));}
  function dividendCurrency337(d){var a=findAt(d.ticker);return a&&a.mercado==='US'?'USD':'BRL';}
  function dividendOptions337(){var opts='<option value="">Escolher destino</option><option value="__CASH_BRL__">Caixa BRL</option><option value="__CASH_USD__">Caixa USD</option>';return opts+(carteira||[]).filter(function(a){return a.tipo!=='caixa';}).map(function(a){return '<option value="'+esc(a.ticker)+'">'+esc(a.ticker)+' — '+esc(a.nome)+'</option>';}).join('');}
  function ensureCash337(market){var ticker=market==='US'?'CAIXA-USD':'CAIXA-BRL',a=findAt(ticker);if(!a){a={ticker:ticker,nome:market==='US'?'Caixa em Dólar':'Caixa Brasil',tipo:'caixa',mercado:market,qtd:0,pmedio:1,pesoAlvo:0,setor:'Liquidez / Caixa'};carteira.push(a);}return a;}
  window.reinvestDividendFromHistory=function(index){
    var d=dividendos[index],sel=document.getElementById('dvDest-'+index);if(!d||!sel||!sel.value){toast('Escolha o destino do valor líquido');return;}if(d.reinvestidoEm){toast('Este dividendo já foi destinado');return;}var target=sel.value,net=dividendNet337(d),origin=dividendCurrency337(d),fx=Number(d.cambio)||fx337();if(net<=0){toast('O dividendo não possui valor líquido');return;}
    if(target==='__CASH_BRL__'||target==='__CASH_USD__'){
      var market=target==='__CASH_USD__'?'US':'BR',cash=ensureCash337(market),amount=origin===market.replace('US','USD').replace('BR','BRL')?net:(market==='US'?net/fx:net*fx),brl=market==='US'?amount*fx:amount;cash.qtd=Number(cash.qtd||0)+amount;cash.pmedio=1;d.reinvestidoEm=cash.ticker;d.reinvestidoData=new Date().toLocaleDateString('pt-BR');aportes.push({tipo:'reinvestimento',data:d.reinvestidoData,ticker:cash.ticker,nome:cash.nome,qtd:amount,preco:1,cambio:market==='US'?fx:1,taxa:0,valorOrig:amount,valorBrl:brl,pmDepois:1,dividendoId:d.id,obs:'Valor líquido de dividendo '+d.ticker});save();renderAll();buildDividendos();toast('Dividendo destinado para '+cash.nome);return;
    }
    var asset=findAt(target);if(!asset)return;var amountTarget=origin===(asset.mercado==='US'?'USD':'BRL')?net:(asset.mercado==='US'?net/fx:net*fx);window.__pendingDividendReinvest={index:index,target:target};openM('aporte');document.getElementById('aTk').value=target;aporteTickerChange();document.getElementById('aValor').value=amountTarget.toFixed(2);document.getElementById('aCambio').value=fx.toFixed(4);var price=Number((getP(asset)||{}).preco||asset.pmedio||0);document.getElementById('aPreco').value=price?price.toFixed(4):'';document.getElementById('aObs').value='Reinvestimento do dividendo líquido de '+d.ticker;aporteCalc();toast('Confira os dados e confirme o reinvestimento');
  };
  window.buildDividendos=function(){
    var data=document.getElementById('dvData');if(data&&!data.value)data.value=new Date().toLocaleDateString('pt-BR');var opts=(carteira||[]).filter(function(a){return a.tipo!=='caixa';}).map(function(a){return '<option value="'+esc(a.ticker)+'">'+esc(a.ticker)+' — '+esc(a.nome)+'</option>';}).join(''),assetSel=document.getElementById('dvAtivo');if(assetSel){var keep=assetSel.value;assetSel.innerHTML=opts;if(keep)assetSel.value=keep;}dvAtivoChange();
    var totalBRL=0,impBRL=0,changed=false;(dividendos||[]).forEach(function(d,i){if(!d.id){d.id='dv-'+Date.now()+'-'+i;changed=true;}var a=findAt(d.ticker),fx=a&&a.mercado==='US'?(Number(d.cambio)||fx337()):1;totalBRL+=Number(d.valor||0)*fx;impBRL+=Number(d.imposto||0)*fx;});if(changed)save(true);var e=document.getElementById('dvTotalBRL');if(e)e.textContent=money(totalBRL);e=document.getElementById('dvTotalUSD');if(e)e.textContent=fU(totalBRL/fx337());e=document.getElementById('dvTotalImp');if(e)e.textContent=money(impBRL);
    var h=(dividendos||[]).map(function(d,index){var a=findAt(d.ticker),us=a&&a.mercado==='US',net=dividendNet337(d),done=!!d.reinvestidoEm;return '<article class="nc337-dividend"><div class="nc337-dividend-main"><div><span class="tk">'+esc(d.ticker)+'</span><strong>'+esc(d.data)+' · '+esc(d.periodo)+'</strong><small>'+esc(d.obs||'Sem observação')+'</small></div><div><span>Bruto <b>'+(us?fU(d.valor):fR(d.valor))+'</b></span><span>Imposto <b class="neg">'+(us?fU(d.imposto||0):fR(d.imposto||0))+'</b></span><span>Líquido <b class="pos">'+(us?fU(net):fR(net))+'</b></span></div></div><div class="nc337-reinvest '+(done?'done':'')+'">'+(done?'<span>✓ Destinado para <b>'+esc(d.reinvestidoEm)+'</b> em '+esc(d.reinvestidoData||'')+'</span>':'<span>Destino do valor líquido</span><select id="dvDest-'+index+'">'+dividendOptions337()+'</select><button onclick="reinvestDividendFromHistory('+index+')">Reinvestir</button>')+'</div></article>';}).reverse().join('');var root=document.getElementById('dvHistorico');if(root)root.innerHTML=h||'<p class="empty">Nenhum dividendo registrado.</p>';
  };
  var saveAporte337=saveAporte;saveAporte=function(){var pending=window.__pendingDividendReinvest,before=(aportes||[]).length;saveAporte337();if(pending&&(aportes||[]).length>before){var d=dividendos[pending.index];if(d){d.reinvestidoEm=pending.target;d.reinvestidoData=new Date().toLocaleDateString('pt-BR');var last=aportes[aportes.length-1];last.tipo='reinvestimento';last.dividendoId=d.id;save(true);}window.__pendingDividendReinvest=null;}};
  var regDiv337=regDiv;regDiv=function(){var before=(dividendos||[]).length;regDiv337();if((dividendos||[]).length>before){var d=dividendos[dividendos.length-1];if(!d.id)d.id='dv-'+Date.now();save(true);buildDividendos();}};
  var open337=nc15OpenPage;nc15OpenPage=function(id,button){open337(id,button);if(id==='vg')setTimeout(buildDecisionDashboard337,0);if(id==='at')setTimeout(simplifyPortfolio337,0);if(id==='dv')setTimeout(buildDividendos,0);};
  document.addEventListener('DOMContentLoaded',function(){document.body.classList.add('nc337');setTimeout(function(){simplifyPortfolio337();buildDecisionDashboard337();},1000);});
})();

// Final precedence for Build 33.8 after every compatibility layer.
(function(){
  if(window.nc338Dashboard)nc15BuildDashboard=window.nc338Dashboard;
  if(window.nc338Portfolio)buildAtivos=window.nc338Portfolio;
  if(window.nc338Rebalance)buildRebalance=window.nc338Rebalance;
  if(window.nc338RebalancePDF)exportRebalancePDF=window.nc338RebalancePDF;
  var dividendHistory338=buildDividendos;
  buildDividendos=function(){dividendHistory338();document.querySelectorAll('#dvHistorico .nc337-dividend').forEach(function(card,reverseIndex){var index=dividendos.length-1-reverseIndex,bar=card.querySelector('.nc337-dividend-main>div:last-child');if(bar&&!bar.querySelector('.nc338-div-actions')){var actions=document.createElement('div');actions.className='nc338-div-actions';actions.innerHTML='<button onclick="editDividend338('+index+')">Editar</button><button class="danger" onclick="deleteDividend338('+index+')">Excluir</button>';bar.appendChild(actions);}});};
  var openPages338=nc15OpenPage;
  nc15OpenPage=function(id,button){openPages338(id,button);document.body.classList.toggle('nc338-portfolio-mode',id==='at');if(id==='vg'&&window.nc338Dashboard)setTimeout(window.nc338Dashboard,0);if(id==='at'&&window.nc338Portfolio)setTimeout(function(){window.nc338Portfolio(totais().pat);},0);if(id==='fi')setTimeout(function(){try{buildNorthReferenceIntelligence();var needle=document.getElementById('ncGaugeNeedle');if(needle)needle.style.display='none';var svg=document.querySelector('#pg-fi .nc-gauge-panel svg'),score=qualityIndex(),angle=Math.PI-(score/100)*Math.PI,cx=230+175*Math.cos(angle),cy=225-175*Math.sin(angle),dot=document.getElementById('nc338GaugeDot');if(!dot&&svg){dot=document.createElementNS('http://www.w3.org/2000/svg','circle');dot.id='nc338GaugeDot';dot.setAttribute('r','10');dot.setAttribute('class','nc338-gauge-dot');svg.appendChild(dot);}if(dot){dot.setAttribute('cx',cx.toFixed(1));dot.setAttribute('cy',cy.toFixed(1));}}catch(e){}},0);if(id==='dv')setTimeout(buildDividendos,0);};
  document.addEventListener('DOMContentLoaded',function(){document.body.classList.add('nc338');setTimeout(function(){try{nc15BuildDashboard();}catch(e){console.error('Build 33.8 final',e);}},1250);});
})();

// ── BUILD 33.9 · MARKET & DECISION CORE ──────────────
(function(){
'use strict';
var fallbackFetch=fetchAll;
function fx39(){var v=Number(usdBrl||0);try{if(!(v>3&&v<10))v=Number(window.nc32EffectiveFx());}catch(e){}return v>3&&v<10?v:5.08;}
function q39(a){var p=getP(a)||{},v=Number(p.preco);return v>0?v:Number(a.pmedio||0);}
function afx39(a){if(a.mercado!=='US')return 1;var v=Number(cambioMedioHistorico(a.ticker));return v>3&&v<10?v:fx39();}
function val39(a){var q=Number(a.qtd||0);return a.tipo==='caixa'?q*(a.mercado==='US'?fx39():1):q*q39(a)*(a.mercado==='US'?fx39():1);}
function cost39(a){return Number(a.qtd||0)*Number(a.pmedio||0)*(a.mercado==='US'?afx39(a):1);}
function pc39(v){return (Number(v)>=0?'+':'')+Number(v||0).toFixed(2).replace('.',',')+'%';}
function metrics39(){var m={pat:0,cost:0,us:0,usVal:0,usCost:0,covered:0,total:0,classes:{}};(carteira||[]).forEach(function(a){var v=val39(a),c=cost39(a),k=rbClassOf(a),p=getP(a)||{};m.pat+=v;m.cost+=c;m.classes[k]=(m.classes[k]||0)+v;if(a.tipo!=='caixa'&&!String(a.ticker).startsWith('TESE-')){m.total++;if(p.fonte&&p.fonte!=='PM'&&p.fonte!=='LOCAL')m.covered++;}if(a.mercado==='US'){m.us+=v;m.usVal+=Number(a.qtd||0)*(a.tipo==='caixa'?1:q39(a));m.usCost+=Number(a.qtd||0)*Number(a.pmedio||0);}});m.gain=m.pat-m.cost;m.ret=m.cost?m.gain/m.cost*100:0;m.usGain=m.usVal-m.usCost;m.usRet=m.usCost?m.usGain/m.usCost*100:0;return m;}
fetchAll=async function(){var items=(carteira||[]).filter(function(a){return a.tipo!=='caixa'&&!String(a.ticker).startsWith('TESE-');});try{var symbols=items.map(function(a){return a.ticker;}).join(','),br=items.filter(function(a){return a.mercado!=='US';}).map(function(a){return a.ticker;}).join(','),r=await fetch('/api/market?symbols='+encodeURIComponent(symbols)+'&br='+encodeURIComponent(br),{cache:'no-store'});if(!r.ok)throw Error(r.status);var d=await r.json(),updated=0;if(Number(d.fx)>3&&Number(d.fx)<10){usdBrl=Number(d.fx);fxSource=(d.fxSource||'North Engine')+' via North Engine';fxIsFresh=true;camUpd=d.updatedAt||new Date().toISOString();}items.forEach(function(a){var p=d.quotes&&d.quotes[a.ticker];if(p&&Number(p.price)>0){precos[a.ticker]={preco:Number(p.price),var:Number(p.changePercent)||0,moeda:p.currency||(a.mercado==='US'?'USD':'BRL'),semDados:false,stale:false,fonte:p.source||'North Engine',ts:(Number(p.marketTime)||Date.now()/1000)*1000};updated++;}else if(precos[a.ticker])precos[a.ticker].stale=true;});saveQuoteCache();return{updated:updated,total:items.length,fxOk:Number(d.fx)>3};}catch(e){console.warn('Market Core fallback',e);return fallbackFetch();}};
pBRL=function(a){return a.tipo==='caixa'?(a.mercado==='US'?fx39():1):q39(a)*(a.mercado==='US'?fx39():1);};pmBRL=function(a){return Number(a.pmedio||0)*(a.mercado==='US'?afx39(a):1);};
function regions39(m){var r={Brasil:0,EUA:0,Canadá:0,Europa:0,Índia:0,Ásia:0,Outros:0};function add(k,v){r[k]+=v;}(carteira||[]).forEach(function(a){var v=val39(a),t=String(a.ticker).toUpperCase();if(a.mercado!=='US')return add('Brasil',v);if(t==='INDA')return add('Índia',v);if(t==='RACE')return add('Europa',v);if(t==='VXUS'){add('Europa',v*.42);add('Ásia',v*.32);add('Canadá',v*.08);return add('Outros',v*.18);}if(t==='URA'||t==='SHLD'){add('EUA',v*.52);add('Canadá',v*.12);add('Europa',v*.2);add('Ásia',v*.1);return add('Outros',v*.06);}add('EUA',v);});return Object.keys(r).filter(function(k){return r[k]>0;}).map(function(k){return '<div><span>'+k+'</span><b>'+((r[k]/(m.pat||1))*100).toFixed(1).replace('.',',')+'%</b><small>'+fR(r[k])+'</small></div>';}).join('');}
function allocations39(m){var rows=[['Brasil',m.pat-m.us,'#1fd69a'],['Exterior',m.us,'#2785ff'],['Renda fixa BR',m.classes.BR_RENDA||0,'#bd78ff'],['ETFs internacionais',m.classes.US_ETF||0,'#3394ff'],['Ações internacionais',m.classes.US_ACAO||0,'#20ca83']];return rows.map(function(x){var p=m.pat?x[1]/m.pat*100:0;return '<div class="nc339-alloc"><span>'+x[0]+'</span><b>'+p.toFixed(1).replace('.',',')+'%</b><small>'+fR(x[1])+'</small><i><em style="width:'+Math.min(100,p)+'%;background:'+x[2]+'"></em></i></div>';}).join('');}
function heat39(m){return (carteira||[]).filter(function(a){return a.tipo!=='caixa'&&val39(a)>0;}).map(function(a){return{a:a,v:val39(a),s:strategyScore(a)};}).sort(function(a,b){return b.v-a.v;}).map(function(x){var cls=x.s>=80?'x':x.s>=70?'g':x.s>=60?'a':'r';return '<button class="'+cls+'" onclick="nc15OpenPage(\'at\')"><strong>'+esc(x.a.ticker)+'</strong><b>'+(x.v/(m.pat||1)*100).toFixed(1)+'%</b><span>'+fR(x.v)+'</span><small>Score '+Math.round(x.s)+'</small></button>';}).join('');}
function dash39(){var root=document.getElementById('pg-vg');if(!root)return;var m=metrics39(),largest=(carteira||[]).filter(function(a){return val39(a)>0;}).sort(function(a,b){return val39(b)-val39(a);})[0],coverage=m.total?m.covered/m.total*100:100,score=qualityIndex(),last=camUpd?new Date(camUpd):new Date(),h=document.getElementById('nc15HeaderUsd');if(h)h.textContent=fx39().toFixed(4).replace('.',',');root.innerHTML='<section class="nc339-head"><div><span>MARKET & DECISION CORE · BUILD 33.9</span><h1>Dashboard patrimonial</h1><p>Resultado real, efeito cambial, exposição e prioridades em uma única leitura.</p></div><div class="nc339-live"><i></i><div><span>USD/BRL</span><strong>R$ '+fx39().toFixed(4).replace('.',',')+'</strong><small>'+esc(fxSource||'North Engine')+' · '+last.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})+'</small></div><button onclick="refresh()">↻ Atualizar</button></div></section><section class="nc339-kpis"><article><span>Carteira BRL</span><strong>'+fR(m.pat)+'</strong><small>Custo '+fR(m.cost)+'</small></article><article><span>Carteira USD</span><strong>'+fU(m.pat/fx39())+'</strong><small>Equivalente pelo dólar atual</small></article><article><span>Ganho real BRL</span><strong class="'+fC(m.gain)+'">'+fR(m.gain)+'</strong><small>'+pc39(m.ret)+' sobre custo e dólar médio</small></article><article><span>Ganho internacional USD</span><strong class="'+fC(m.usGain)+'">'+fU(m.usGain)+'</strong><small>'+pc39(m.usRet)+' sem efeito cambial</small></article><article><span>Maior posição</span><strong>'+(largest?esc(largest.ticker):'—')+'</strong><small>'+(largest?(val39(largest)/(m.pat||1)*100).toFixed(1)+'% · '+fR(val39(largest)):'—')+'</small></article><article><span>Cotações de mercado</span><strong>'+m.covered+'/'+m.total+'</strong><small>'+coverage.toFixed(0)+'% atualizadas</small></article></section><section class="nc339-main"><article class="nc339-panel"><header><span>ALOCAÇÃO + EXPOSIÇÃO GEOGRÁFICA</span><h2>Onde está o patrimônio</h2></header><div class="nc339-exposure"><div><img src="assets/world-political.svg" alt="Mapa político mundial com fronteiras nacionais"><div class="nc339-regions">'+regions39(m)+'</div></div><div>'+allocations39(m)+'</div></div></article><article class="nc339-panel nc339-priority"><header><span>LEITURA EXECUTIVA</span><h2>O que exige atenção</h2></header><div class="nc339-score">'+score+'<small>/100</small></div><p>'+northScoreSemantic(score).label+'</p><div><span>Rentabilidade BRL</span><b class="'+fC(m.ret)+'">'+pc39(m.ret)+'</b></div><div><span>Exposição internacional</span><b>'+(m.us/(m.pat||1)*100).toFixed(1)+'%</b></div><div><span>Qualidade das cotações</span><b>'+coverage.toFixed(0)+'%</b></div><button onclick="nc15OpenPage(\'rb\')">Simular próximo aporte</button></article></section><section class="nc339-panel nc339-heat"><header><span>HEATMAP COMPLETO</span><h2>Peso e qualidade de todos os ativos</h2></header><div>'+heat39(m)+'</div></section>';}
function portfolio39(){var root=document.getElementById('tbAt');if(!root)return;var m=metrics39(),head=root.closest('table').querySelector('thead');head.innerHTML='<tr><th>Ativo</th><th>Qtd.</th><th>PM USD</th><th>USD médio</th><th>Cotação atual</th><th>Valor atualizado</th><th>Ganho USD</th><th>Rent. USD</th><th>USD/BRL atual</th><th>Ganho BRL</th><th>Rent. BRL</th><th>Peso</th><th>Ações</th></tr>';root.innerHTML=(carteira||[]).map(function(a,i){var us=a.mercado==='US',q=Number(a.qtd||0),p=q39(a),v=val39(a),c=cost39(a),gu=us?q*(p-Number(a.pmedio||0)):0,ru=us&&q*a.pmedio?gu/(q*a.pmedio)*100:0,gb=v-c,rb=c?gb/c*100:0,src=getP(a)||{};return '<tr><td><div class="nc338-asset"><span class="tk">'+esc(a.ticker)+'</span><strong>'+esc(a.nome||'')+'</strong></div></td><td>'+q.toFixed(6)+'</td><td>'+(us?fU(a.pmedio):'—')+'</td><td>'+(us?'R$ '+afx39(a).toFixed(4):'—')+'</td><td>'+(us?fU(p):fR(p))+'<small class="nc338-source">'+esc(src.fonte||'PM')+(src.stale?' · cache':'')+'</small></td><td><b>'+fR(v)+'</b>'+(us?'<small class="nc338-source">'+fU(q*p)+'</small>':'')+'</td><td class="'+fC(gu)+'">'+(us?fU(gu):'—')+'</td><td class="'+fC(ru)+'">'+(us?pc39(ru):'—')+'</td><td>R$ '+fx39().toFixed(4)+'</td><td class="'+fC(gb)+'">'+fR(gb)+'</td><td class="'+fC(rb)+'">'+pc39(rb)+'</td><td>'+(v/(m.pat||1)*100).toFixed(2)+'%</td><td><div class="nc338-actions"><button onclick="openM(\'edit\','+i+')">Editar</button><button class="danger" onclick="delAt('+i+')">Excluir</button></div></td></tr>';}).join('');}
var prevRb=buildRebalance;buildRebalance=function(){prevRb();var p=window.__rebalancePlan;if(!p)return;var rate=Number((document.getElementById('rbCambio')||{}).value)||Number(p.fx)||fx39();p.fx=rate;p.items.sort(function(a,b){return b.amount-a.amount;});p.items.forEach(function(x){x.amountUsd=x.asset.mercado==='US'?x.amount/rate:0;});document.querySelectorAll('#tbReb tr').forEach(function(row){var t=row.querySelector('.tk');if(!t)return;var x=p.items.find(function(i){return i.asset.ticker===t.textContent.trim();}),c=row.querySelectorAll('td');if(x&&x.asset.mercado==='US'&&c[3])c[3].innerHTML='<b>'+fU(x.amountUsd)+'</b><small class="nc336-br-ref">'+fR(x.amount)+' opcional</small>';});};
function hist39(){var tb=document.getElementById('tbHist');if(!tb)return;var at=(document.getElementById('hsFiltAt')||{}).value||'',tp=(document.getElementById('hsFiltTp')||{}).value||'',b=((document.getElementById('hsFiltBusca')||{}).value||'').toUpperCase(),rows=(aportes||[]).map(function(x,i){return{x:x,i:i};}).filter(function(v){return(!at||v.x.ticker===at)&&(!tp||v.x.tipo===tp)&&(!b||String(v.x.ticker).toUpperCase().includes(b));}).reverse(),tr=tb.closest('table').querySelector('thead tr');if(tr&&!tr.querySelector('.nc339-ha')){var th=document.createElement('th');th.className='nc339-ha';th.textContent='Ações';tr.appendChild(th);}tb.innerHTML=rows.map(function(v){var x=v.x,s=x.tipo==='venda';return '<tr><td>'+esc(x.data)+'</td><td><span class="tk">'+esc(x.ticker)+'</span></td><td>'+esc(x.nome||'')+'</td><td>'+esc(x.tipo||'aporte')+'</td><td>'+Number(x.qtd||0).toFixed(8)+'</td><td>'+Number(x.preco||0).toFixed(4)+'</td><td>'+Number(x.cambio||0).toFixed(4)+'</td><td>'+fR(x.taxa||0)+'</td><td>'+Number(x.valorOrig||0).toFixed(2)+'</td><td>'+fR(x.valorBrl||0)+'</td><td>'+(s?fR(x.lucro||0):'—')+'</td><td>'+(s?fR(x.ir||0):'—')+'</td><td>'+(s?fR(x.liquido||0):'—')+'</td><td>'+Number(x.pmDepois||0).toFixed(4)+'</td><td>'+esc(x.obs||'')+'</td><td><div class="nc338-actions"><button onclick="editHistory39('+v.i+')">Editar</button><button class="danger" onclick="deleteHistory39('+v.i+')">Excluir</button></div></td></tr>';}).join('')||'<tr><td colspan="16" class="empty">Nenhuma movimentação encontrada.</td></tr>';}
window.editHistory39=function(i){var x=aportes[i];if(!x)return;var d=prompt('Data (dd/mm/aaaa):',x.data||'');if(d===null||!validDate(d)){if(d!==null)toast('Data inválida');return;}var p=prompt('Preço:',String(x.preco||0));if(p===null)return;var c=prompt('Câmbio:',String(x.cambio||1));if(c===null)return;x.data=d;x.preco=Math.max(0,Number(String(p).replace(',','.'))||0);x.cambio=Math.max(0,Number(String(c).replace(',','.'))||1);x.obs=prompt('Observação:',x.obs||'')||'';x.valorBrl=Number(x.valorOrig||0)*x.cambio+Number(x.taxa||0);save();hist39();toast('Histórico atualizado');};
window.deleteHistory39=function(i){if(!aportes[i]||!confirm('Excluir esta movimentação do histórico? A posição atual será preservada.'))return;aportes.splice(i,1);save();hist39();toast('Movimentação excluída');};
nc15BuildDashboard=dash39;buildAtivos=portfolio39;var oldHist=buildHistorico;buildHistorico=function(){oldHist();hist39();};var open39=nc15OpenPage;nc15OpenPage=function(id,b){open39(id,b);document.body.classList.toggle('nc338-portfolio-mode',id==='at');if(id==='vg')setTimeout(dash39,0);if(id==='at')setTimeout(portfolio39,0);if(id==='hs')setTimeout(hist39,0);};document.addEventListener('DOMContentLoaded',function(){document.body.classList.add('nc339');setTimeout(function(){dash39();NorthEngine.update('startup');},1200);});
})();
