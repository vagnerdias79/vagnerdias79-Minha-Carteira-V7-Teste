...
function buildRebalance(){
  ...
  if(aporte>0){
    if(totalDef>0)deficits.forEach(function(x){classAlloc[x.key]=aporte*x.deficit/totalDef;});
    else{
      var metaSum=classes.reduce(function(s,k){return s+(meta[k]||0);},0)||1;
      classes.forEach(function(k){classAlloc[k]=aporte*(meta[k]||0)/metaSum;});
    }
  }else classes.forEach(function(k){classAlloc[k]=0;});
  ...
  document.getElementById('rbPrior').textContent=items.length?items.slice().sort(function(a,b){return b.amount-a.amount;})[0].asset.ticker:'—';
  ...
  document.getElementById('rbSobra').textContent=fR(sobra+pendingTotal);
  ...
  else resumo.innerHTML='<strong>Plano recomendado — '+sorted.length+' ativo(s)</strong>'+sorted.slice(0,8).map(function(x,i){return '<div class="buy-line"><span><span class="rank">'+(i+1)+'</span>'+esc(x.asset.ticker)+' · '+esc(x.asset.nome)+'</span><strong>'+fR(x.amount)+' · '+x.qty.toFixed(6)+' cota(s)</strong></div>';}).join('');
  ...
  tbody.innerHTML=sorted.length?sorted.map(function(x){
    ...
    return '<tr><td><span class="priority-pill priority-high">Comprar</span></td><td><span class="tk">'+esc(a.ticker)+'</span><div style="font-size:10px;color:var(--text2)">'+esc(a.nome)+'</div></td><td>'+esc(rbClassLabel(k))+'</td><td>'+fR(v)+'</td><td>'+actual.toFixed(2)+'%</td><td>'+classMeta.toFixed(2)+'%</td><td class="'+fC(actual-classMeta)+'">'+fP(actual-classMeta)+'</td><td class="reb-buy">'+fR(x.amount)+'</td><td>'+x.qty.toFixed(6)+'</td><td><span class="reb-buy">COMPRAR</span></td></tr>';
  }).join(''):'<tr><td colspan="10" class="empty">Nenhuma compra sugerida para o escopo selecionado.</td></tr>';

  window.__rebalancePlan={aporte:aporte,items:sorted,pending:pending,sobra:sobra};
}
