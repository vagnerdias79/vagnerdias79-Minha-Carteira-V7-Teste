...
function syncPolicyGlobals(){
  ...
  METF=Number(policy.US.etf)||0; MACAO=Number(policy.US.acao)||0;
}
...
function mergePolicy(saved){
  ...
  ['etf','acao'].forEach(function(k){if(saved.US&&saved.US[k]!==undefined)base.US[k]=Number(saved.US[k]);});
  return base;
}
function load() {
  try {
...
function buildExecutiveKpis(pat){
  ...
  x=document.getElementById('titleUS');if(x)x.textContent='EUA — Meta '+METF+'% ETF / '+MACAO+'% Ação';
...
function currentPolicyFromForm(){
  return {
    ...
    US:{etf:policyValue('polUSETF'),acao:policyValue('polUSAcao')}
  };
}
...
function buildPolicy(){
  ...
  setPolicyInput('polUSETF',policy.US.etf);setPolicyInput('polUSAcao',policy.US.acao);
  previewPolicy();
}
...
function previewPolicy(){
  ...
  var us=policyGroupValid([p.US.etf,p.US.acao]);
  ...
  setPolicyBar('polBarUSETF',p.US.etf,'#f59e0b');setPolicyBar('polBarUSAcao',p.US.acao,'#4f8ef7');
