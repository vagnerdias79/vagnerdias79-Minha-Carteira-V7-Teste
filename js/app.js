...
var MUS=30, MBR=70, METF=55, MACAO=45, MRF=60, MABR=30, MCAIXA=10;
...
function load() {
  try {
    ...
    if (p) { carteira = p.carteira || JSON.parse(JSON.stringify(PADRAO)); dividendos = p.dividendos || []; aportes = p.aportes || []; }
    else { carteira = JSON.parse(JSON.stringify(PADRAO)); dividendos = []; aportes = []; }
  } catch(e) { carteira = JSON.parse(JSON.stringify(PADRAO)); dividendos = []; aportes = []; }
}
function save() {
  try { localStorage.setItem(SK, JSON.stringify({v:6, carteira:carteira, dividendos:dividendos, aportes:aportes})); toast('Salvo'); } catch(e) {}
}
...
function imp(e) {
  ...
  r.onload = function(ev) {
    try {
      ...
      carteira = d.carteira || d; dividendos = d.dividendos || []; aportes = d.aportes || [];
...
// ── PRECOS ────────────────────────────────────────────
function pBRL(a) { var p=precos[a.ticker]; if(!p) return a.pmedio*(a.mercado==='US'?usdBrl:1); return p.moeda==='USD'?p.preco*usdBrl:p.preco; }
function pmBRL(a) { return a.mercado==='US'?a.pmedio*usdBrl:a.pmedio; }
function getP(a) { return precos[a.ticker]||{preco:a.pmedio,var:0,moeda:a.mercado==='US'?'USD':'BRL'}; }
...
function buildExecutiveKpis(pat){
  var us=0,etf=0,acoes=0;
  carteira.forEach(function(at){
    var v=at.qtd*pBRL(at);
    if(at.mercado==='US'){
      us+=v;
      if(at.tipo==='etf')etf+=v; else acoes+=v;
    }
  });
  var pExt=pat?us/pat*100:0, pEtf=us?etf/us*100:0, pAcoes=us?acoes/us*100:0;
  var desv=(Math.abs(pExt-MUS)/Math.max(MUS,1)+Math.abs(pEtf-METF)/Math.max(METF,1)+Math.abs(pAcoes-MACAO)/Math.max(MACAO,1))/3;
  ...
  bar('barExterior',pExt/MUS*100);bar('barEtf',pEtf/METF*100);bar('barAcao',pAcoes/MACAO*100);bar('barAderencia',ader);
}
...
function buildCharts(pat) {
  var us=0,br=0,etf=0,ac=0,rf=0,acbr=0;
  var etfM={},acUSM={},rfM={},acBRM={};
  carteira.forEach(function(a){
    var v=a.qtd*pBRL(a);
    if(a.mercado==='US'){us+=v; if(a.tipo==='etf'){etf+=v;etfM[a.ticker]=(etfM[a.ticker]||0)+v;}else{ac+=v;acUSM[a.ticker]=(acUSM[a.ticker]||0)+v;}}
  ...
  cTipo=mkChart('cTipo',cTipo,['ETFs','Acoes'],[etf,ac],['#f59e0b','#4f8ef7'],'EUA',fR(etf+ac));
...
function buildResumo(pat){
  var us=0,br=0,etf=0,ac=0,rf=0,acbr=0,cx=0;
  carteira.forEach(function(a){var v=a.qtd*pBRL(a); if(a.mercado==='US'){us+=v;if(a.tipo==='etf')etf+=v;else ac+=v;}else{br+=v;if(a.tipo==='caixa')cx+=v;else if(a.tipo==='acao'||a.tipo==='etf'||a.tipo==='fii')acbr+=v;else rf+=v;}});
  var pus=pat?us/pat*100:0,pbr=pat?br/pat*100:0,petf=(etf+ac)?etf/(etf+ac)*100:0,pac=(etf+ac)?ac/(etf+ac)*100:0,prf=br?rf/br*100:0,pabr=br?acbr/br*100:0,pcx=br?cx/br*100:0;
  document.getElementById('resAloc').innerHTML=
    ...
    '<div style="font-size:10px;color:var(--text2);text-transform:uppercase;margin:10px 0 2px">Dentro do EUA</div>'+alocRow('ETFs',petf,METF,'#f59e0b')+alocRow('Acoes EUA',pac,MACAO,'#f87172')+
...
function buildAloc(pat){
  ...
  document.getElementById('alUS').innerHTML=barItem('ETFs',petf,METF,'#f59e0b')+barItem('Acoes',pac,MACAO,'#f87172');
...
function backupJson(){var data={v:6,exportadoEm:new Date().toISOString(),carteira:carteira,dividendos:dividendos,aportes:aportes};var blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}),u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download='carteira_backup_'+new Date().toISOString().slice(0,10)+'.json';a.click();setTimeout(function(){URL.revokeObjectURL(u);},1000);toast('Backup JSON criado');}

// ── REBALANCEAMENTO ──────────────────────────────────
function rebAdherence(rows, total, useAfter){
  if(!rows.length||!total)return 0;
  var deviation=rows.reduce(function(sum,r){
    var value=useAfter ? r.v+r.buy : r.v;
    var pct=value/total*100;
    return sum+Math.abs(pct-r.mt);
  },0);
  return Math.max(0,100-deviation/2);
}
function priorityInfo(r,maxBuy){
  if(r.buy<=0.01)return {label:'Sem aporte',cls:'priority-none'};
  var ratio=maxBuy?r.buy/maxBuy:0;
  if(ratio>=.70)return {label:'Muito alta',cls:'priority-very-high'};
  if(ratio>=.35)return {label:'Alta',cls:'priority-high'};
  return {label:'Média',cls:'priority-medium'};
}
function buildRebalance(){
  var tbody=document.getElementById('tbReb'); if(!tbody)return;
  var aporte=Math.max(num('rbAporte'),0),scope=document.getElementById('rbEscopo').value;
  var ativos=carteira.filter(function(a){return (a.pesoAlvo||0)>0&&(scope==='ALL'||a.mercado==='US');});
  var total=ativos.reduce(function(s,a){return s+a.qtd*pBRL(a);},0),metaSum=ativos.reduce(function(s,a){return s+(a.pesoAlvo||0);},0);
  if(!ativos.length||!total||!metaSum){
    tbody.innerHTML='<tr><td colspan="10" class="empty">Cadastre metas e posições para calcular o rebalanceamento.</td></tr>';
    return;
  }
  var finalTotal=total+aporte;
  var rows=ativos.map(function(a){
    var atualV=a.qtd*pBRL(a),meta=(a.pesoAlvo||0)/metaSum,atualPct=atualV/total*100,metaPct=meta*100;
    var deficit=Math.max(finalTotal*meta-atualV,0);
    return {a:a,v:atualV,at:atualPct,mt:metaPct,des:atualPct-metaPct,def:deficit,buy:0,q:0};
  });
  var totalDef=rows.reduce(function(s,r){return s+r.def;},0);
  var alocado=Math.min(aporte,totalDef);
  rows.forEach(function(r){
    r.buy=totalDef?alocado*r.def/totalDef:0;
    r.q=r.buy/(pBRL(r.a)||1);
  });
  rows.sort(function(a,b){return b.buy-a.buy||a.des-b.des;});
  var buys=rows.filter(function(r){return r.buy>0.01;}),maxBuy=buys.length?buys[0].buy:0,sobra=Math.max(aporte-alocado,0);
  var adAntes=rebAdherence(rows,total,false),adDepois=rebAdherence(rows,total+alocado,true),melhora=adDepois-adAntes;
  document.getElementById('rbTotal').textContent=fR(aporte);
  document.getElementById('rbQtd').textContent=String(buys.length);
  document.getElementById('rbPrior').textContent=buys.length?buys[0].a.ticker:'Nenhum';
  document.getElementById('rbAdAntes').textContent=adAntes.toFixed(1)+'%';
  document.getElementById('rbAdDepois').textContent=adDepois.toFixed(1)+'%';
  document.getElementById('rbMelhora').textContent=(melhora>=0?'+':'')+melhora.toFixed(1)+' p.p.';
  document.getElementById('rbSobra').textContent=fR(sobra);
  document.getElementById('rbBarAntes').style.width=Math.min(adAntes,100)+'%';
  document.getElementById('rbBarDepois').style.width=Math.min(adDepois,100)+'%';
  var resumo=document.getElementById('rbResumo');
  if(!aporte){resumo.innerHTML='Informe um aporte para gerar o plano de compra.';}
  else if(!buys.length){resumo.innerHTML='<strong>Nenhuma compra necessária.</strong> As posições com meta definida já estão preenchidas para este escopo.';}
  else{
    resumo.innerHTML='<strong>Plano recomendado — '+buys.length+' ativo(s)</strong>'+buys.slice(0,6).map(function(r,i){return '<div class="buy-line"><span><span class="rank">'+(i+1)+'</span>'+esc(r.a.ticker)+' · '+esc(r.a.nome)+'</span><strong>'+fR(r.buy)+' · '+r.q.toFixed(6)+' cota(s)</strong></div>';}).join('')+(sobra>0.01?'<div style="margin-top:8px">Reserva não alocada: <strong>'+fR(sobra)+'</strong>.</div>':'');
  }
  tbody.innerHTML=rows.map(function(r){
    var pri=priorityInfo(r,maxBuy),action=r.buy>0.01?'<span class="reb-buy">COMPRAR</span>':'<span class="reb-hold">PAUSAR</span>';
    return '<tr><td><span class="priority-pill '+pri.cls+'">'+pri.label+'</span></td><td><span class="tk">'+esc(r.a.ticker)+'</span><div style="font-size:10px;color:var(--text2)">'+esc(r.a.nome)+'</div></td><td>'+esc(r.a.tipo.toUpperCase())+'</td><td>'+fR(r.v)+'</td><td>'+r.at.toFixed(2)+'%</td><td>'+r.mt.toFixed(2)+'%</td><td class="'+fC(r.des)+'">'+fP(r.des)+'</td><td class="reb-buy">'+fR(r.buy)+'</td><td>'+r.q.toFixed(6)+'</td><td>'+action+'</td></tr>';
  }).join('');
  window.__rebalancePlan={aporte:aporte,alocado:alocado,sobra:sobra,aderenciaAntes:adAntes,aderenciaDepois:adDepois,rows:buys};
}
function copyRebalance(){
  var p=window.__rebalancePlan;
  if(!p||!p.aporte){toast('Informe um aporte antes de copiar');return;}
  var lines=['PLANO DE APORTE — MINHA CARTEIRA V7','Aporte: '+fR(p.aporte),'Aderência: '+p.aderenciaAntes.toFixed(1)+'% → '+p.aderenciaDepois.toFixed(1)+'%',''];
  p.rows.forEach(function(r,i){lines.push((i+1)+'. '+r.a.ticker+' — '+fR(r.buy)+' — '+r.q.toFixed(6)+' cota(s)');});
  if(p.sobra>0.01)lines.push('Reserva não alocada: '+fR(p.sobra));
  var text=lines.join('\n');
  if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(text).then(function(){toast('Plano copiado');}).catch(function(){toast('Não foi possível copiar');});}
