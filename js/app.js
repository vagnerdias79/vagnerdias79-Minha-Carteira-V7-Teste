...
function buildNorthExecutiveHome(){
  ...
  if(timeline){
    var events=[];
    (movimentacoes||[]).slice().sort(function(a,b){return String(b.data||"").localeCompare(String(a.data||""));}).slice(0,3).forEach(function(m){events.push({date:m.data||"—",title:(m.tipo||"Movimentação")+" · "+(m.ticker||""),value:m.obs||"Registro patrimonial"});});
...
function buildNorthCurrencyExposure(){
  ...
  if(donut)donut.style.setProperty("--brl",brlPct);
  if(legend)legend.innerHTML=[
    ["BRL",brlPct,"blue"],
    ["USD",usd,"green"]
  ].map(function(x){return `<div><i class="${x[2]}"></i><span>${x[0]}</span><strong>${x[1].toFixed(1)}%</strong></div>`;}).join("");
}
...
function buildNorthTimeline(){
  var root=document.getElementById("northTimeline");if(!root)return;
  var events=[];
  var movs=(movimentacoes||[]).slice().sort(function(a,b){return String(b.data||"").localeCompare(String(a.data||""));}).slice(0,2);
...
function nc15PortfolioGroups(){
  ...
  var groups=[
    ...
    {name:"Renda Fixa",value:carteira.filter(function(a){return a.tipo==="rf";}).reduce(function(s,a){return s+rbValue(a);},0),color:"#f0ae22"},
    {name:"Caixa",value:carteira.filter(function(a){return a.tipo==="caixa";}).reduce(function(s,a){return s+rbValue(a);},0)+(Number(cashUSD)||0)*(Number(usdBrl)||0),color:"#8c50e8"}
  ];
...
function nc15BuildGeo(){
  var pat=totais().pat||1;
  var us=carteira.filter(function(a){return a.mercado==="US";}).reduce(function(s,a){return s+rbValue(a);},0)/pat*100;
  var br=Math.max(0,100-us);
  var values=[["Américas",Math.min(100,us+br*.45),"#176ef5"],["Europa",Math.max(0,br*.18),"#4ecb6a"],["Ásia",Math.max(0,br*.16),"#f2c11e"],["Outros",Math.max(0,100-(us+br*.45+br*.18+br*.16)),"#8e99a8"]];
  var root=document.getElementById("nc15GeoLegend");if(root)root.innerHTML=values.map(function(x){return '<div><i style="background:'+x[2]+'"></i><span>'+x[0]+'</span><strong>'+x[1].toFixed(1)+'%</strong></div>';}).join("");
}
...
function nc15BuildGoals(){
  ...
  var goals=[
    ...
    ["Reserva de Oportunidade",Math.max(0,Number(cashUSD||0)*(usdBrl||1)),200000,"#19ba66"]
  ];
...
function nc15BuildMovers(){
  ...
  var items=carteira.filter(function(a){return a.mercado==="US"&&a.tipo!=="caixa";}).sort(function(a,b){return Math.abs(Number(b.chg||0))-Math.abs(Number(a.chg||0));}).slice(0,5);
  root.innerHTML=items.map(function(a){var ch=Number(a.chg||0);return '<div><span>▦</span><strong>'+esc(a.ticker)+'</strong><em class="'+(ch>=0?'pos':'neg')+'">'+(ch>=0?'▲ ':'▼ ')+Math.abs(ch).toFixed(2)+'%</em></div>';}).join("");
}
...
function nc15BuildDashboard(){
  var t=totais(),score=qualityIndex(),res=t.pat-t.investido,ret=t.investido?res/t.investido*100:0;
  ...
  set("nc15Dollar","R$ "+Number(usdBrl||0).toFixed(2).replace(".",","));set("nc15DollarChange","+0,37%");
  set("nc15HeaderUsd",Number(usdBrl||0).toFixed(2).replace(".",","));set("nc15RibbonUsd",Number(usdBrl||0).toFixed(2).replace(".",","));
  set("nc15CashTotal",nc15Money((Number(cashUSD)||0)*(Number(usdBrl)||0)));
  ...
  var mov=(movimentacoes||[]).slice().sort(function(a,b){return String(b.data||"").localeCompare(String(a.data||""));})[0];
  set("nc15LastContribution",mov?nc15Money(Number(mov.totalBRL||mov.valorBRL||0)):"—");set("nc15LastContributionDate",mov?mov.data:"—");
  ...
  nc15Sparkline("nc15DollarSpark",[5.18,5.22,5.19,5.25,5.21,5.28,5.41,5.35,5.43],"#29c76f");
  nc15BuildAllocation();nc15BuildGeo();nc15BuildEvolution();nc15BuildRadar();nc15BuildEvents();nc15BuildGoals();nc15BuildHeatmap();nc15BuildInsights();nc15BuildAlerts();nc15BuildMovers();
}
