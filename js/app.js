  ...
  if(!aporte)resumo.innerHTML='Informe um aporte para gerar o plano de compra.';
    ...
    function group(title,arr){
      ...
      return '<div class="rb-group-title">'+title+'</div>'+
        arr.map(function(x,i){return '<div class="buy-line"><span><span class="rank">'+(i+1)+'</span>'+esc(x.asset.ticker)+' · '+esc(x.asset.nome)+'</span><strong>'+fR(x.amount)+' · '+x.qty.toFixed(6)+' cota(s)</strong></div>';}).join('');
    }
