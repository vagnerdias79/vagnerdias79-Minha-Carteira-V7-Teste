...
(function(){
  ...
  refresh=async function(){
    setSt('l','Atualizando dados…');var br=document.getElementById('btnR');if(br)br.disabled=true;
    var timed=false,timer=setTimeout(function(){timed=true;try{renderAll();}catch(e){}setSt('e','Dados locais carregados · atualização online indisponível');if(br)br.disabled=false;},14000);
    try{var sync=await fetchAll();if(timed)return;clearTimeout(timer);renderAll();var t=new Date();var msg='Atualizado às '+t.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})+' · '+sync.updated+'/'+sync.total+' ativos';if(!sync.fxOk)msg+=' · câmbio local';setSt('ok',msg);}
    catch(e){if(!timed){clearTimeout(timer);try{renderAll();}catch(_){}setSt('e','Dados locais carregados · clique em ↻ para tentar novamente');}}
  ...
  aporteTickerChange=function(){
    ...
    var info=document.getElementById('aInfoBox');if(cdb&&info){info.innerHTML='<strong>'+esc(a.ticker)+'</strong> — '+esc(a.nome)+'<br>Saldo estimado: '+fR(Number(a.qtd||0)*Number(a.pmedio||0))+'<span class="nc332-cdb-note">Informe apenas o valor do novo aporte. O sistema registra uma nova unidade de aplicação internamente.</span>';}
    aporteCalc();
  };
  aporteCalc=function(){
  ...
  saveAporte=function(){
    ...
    if(isCdb(a)){var oldQtd=Number(a.qtd||0),oldCost=oldQtd*Number(a.pmedio||0),qtd=1,preco=valor;a.qtd=oldQtd+1;a.pmedio=(oldCost+valor)/a.qtd;aportes.push({tipo:'aporte',data:data,ticker:a.ticker,nome:a.nome,qtd:1,preco:valor,cambio:1,taxa:taxa,valorOrig:valor,valorBrl:valor+taxa,pmDepois:a.pmedio,modalidade:'aporte-pre-definido',obs:document.getElementById('aObs').value.trim()});save();closeM();renderAll();toast('Aporte em CDB registrado');return;}
    var preco=num('aPreco');if(preco<=0){toast('Preencha o preço');return;}var cambio=a.mercado==='US'?num('aCambio'):1;if(cambio<=0){toast('Informe o câmbio');return;}var qtd=valor/preco,oldQtd=a.qtd,oldCost=oldQtd*a.pmedio;a.qtd=oldQtd+qtd;a.pmedio=a.qtd?(oldCost+valor)/a.qtd:0;aportes.push({tipo:'aporte',data:data,ticker:a.ticker,nome:a.nome,qtd:qtd,preco:preco,cambio:cambio,taxa:taxa,valorOrig:valor,valorBrl:valor*cambio+taxa,pmDepois:a.pmedio,obs:document.getElementById('aObs').value.trim()});save();closeM();renderAll();toast('Aporte registrado');
  };
  ...
  document.addEventListener('DOMContentLoaded',function(){document.body.classList.add('nc332');var s=document.getElementById('stlbl');if(s&&s.textContent.indexOf('Carregando')>=0)s.textContent='Dados locais prontos';setTimeout(function(){try{nc15BuildDashboard();}catch(e){}},500);});
})();
