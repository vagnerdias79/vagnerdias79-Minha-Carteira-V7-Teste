...
function showPg(id, el) {
  ...
  if (id === 'fi'){buildFamilyIntelligence();setTimeout(function(){buildNorthScore2();buildNorthHeatmap();buildNorthTimeline();},0);}if (id === 'go'){buildNorthGoals();setTimeout(buildNorthChecklist,0);}if (id === 'vg') setTimeout(function(){buildNorthMissionControl();buildNorthMissionAI();},0);if (id === 'na') setTimeout(buildNorthAI,0);if (id === 'vg') setTimeout(function(){buildNorthMissionControl();buildNorthMissionAI();},0);if (id === 'na') setTimeout(buildNorthAI,0);
...
function buildFamilyIntelligence(){
  ...
  txt('fiQualityLabel',q>=80?'Excelente':q>=65?'Bom':q>=50?'Moderado':'Revisar');
...
function buildNorthMissionControl(){var e=document.getElementById("northGreeting");if(e)e.textContent=northTimeGreeting();var s=qualityIndex(),l=s>=80?"Excelente":s>=65?"Bom":s>=50?"Moderado":"Revisar";var fs=document.getElementById("northFamilyScore"),fl=document.getElementById("northFamilyLabel");if(fs)fs.textContent=s+"/100";if(fl)fl.textContent=l;var cur=rbCurrentByClass(),meta=rbClassMeta(),pat=totais().pat||1;var gaps=Object.keys(meta).map(function(k){return{key:k,gap:(meta[k]||0)-((cur[k]||0)/pat*100)};}).sort(function(a,b){return b.gap-a.gap;});var top=gaps[0],p=document.getElementById("northPriority"),r=document.getElementById("northPriorityReason");if(p)p.textContent=top&&top.gap>0?rbClassLabel(top.key):"Carteira alinhada";if(r)r.textContent=top&&top.gap>0?top.gap.toFixed(1)+" p.p. abaixo da meta":"sem déficit relevante";var i=document.getElementById("northMissionInsight");if(i)i.textContent=s>=80?"Sua carteira está bem estruturada. Direcione o próximo aporte aos maiores déficits.":s>=65?"A carteira está consistente, mas ainda há desvios relevantes de alocação.":"A carteira exige atenção em concentração, diversificação ou aderência à política.";}
