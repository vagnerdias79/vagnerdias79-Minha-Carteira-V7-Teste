...
function showPg(id, el) {
  ...
  if (id === 'fi') buildFamilyIntelligence();
...
function buildStrategy(){
  ...
  root.innerHTML=list.sort(function(a,b){return strategyScore(b)-strategyScore(a);}).map(function(a){
    var s=strategyScore(a),id='role-'+a.ticker.replace(/[^a-zA-Z0-9]/g,'_');
    ...
    return '<div class="strategy-card">'+
      '<div class="strategy-card-head"><div><span class="strategy-ticker">'+esc(a.ticker)+'</span><small>'+esc(a.nome)+'</small></div><div class="strategy-score '+(s>=80?'score-good':s>=65?'score-mid':'score-low')+'">'+s+'</div></div>'+
      '<div class="strategy-role"><label>Papel na carteira</label><textarea id="'+id+'" rows="2">'+esc(strategyRole(a))+'</textarea><button class="bs" onclick="saveStrategyRole(\''+String(a.ticker).replace(/'/g,"\\'")+'\')">Salvar tese</button></div>'+
      '<div class="conviction-control"><label>Convicção</label><div><input id="conv-'+a.ticker.replace(/[^a-zA-Z0-9]/g,'_')+'" type="number" min="0" max="100" step="1" value="'+convictionOf(a)+'"><button class="bs" onclick="saveConviction(\''+String(a.ticker).replace(/'/g,"\\'")+'\')">Salvar</button></div><small>'+convictionLabel(convictionOf(a))+'</small></div>'+
      '<div class="strategy-metrics">'+
        '<div><span>Classe</span><strong>'+esc(rbClassLabel(k))+'</strong></div>'+
        '<div><span>Classe atual/meta</span><strong>'+classCurrent.toFixed(1)+'% / '+classTarget.toFixed(1)+'%</strong></div>'+
        '<div><span>Ativo atual/meta</span><strong>'+assetCurrent.toFixed(2)+'% / '+assetTarget.toFixed(2)+'%</strong></div>'+
      '</div>'+
      '<div class="strategy-foot">'+strategyScoreLabel(s)+'</div>'+
    '</div>';
...
function buildFamilyIntelligence(){
  ...
  txt('fiQualityLabel',q>=85?'Excelente':q>=75?'Muito bom':q>=65?'Bom, com ajustes':'Revisar estrutura');
...
function buildRebalance(){
  ...
  if(topBox){
    if(topPriority){
      var rs=recommendationScore(topPriority),rating=purchaseRating(rs);
      topBox.innerHTML='<div class="top-rec-badge">COMPRAR AGORA</div>'+
        '<div class="top-rec-main"><div><span>'+esc(topPriority.asset.ticker)+'</span><small>'+esc(topPriority.asset.nome)+'</small></div><strong>'+rs+'/100</strong></div>'+
