...
function strategyScore(a){
  ...
  return Math.max(0,Math.min(100,Math.round(classScore*.45+assetScore*.4+roleScore*.15-cashPenalty)));
}
...
function recommendationScore(item){
  ...
  var base=strategyScore(a);
  var score=base*.45+Math.min(100,classDeficit*8)*.35+Math.min(100,allocationShare*6)*.20;
  if(classCurrent>classMeta&&classMeta>0)score-=35;
...
function buildRebalance(){
  ...
  if(topBox){
    if(topPriority){
      var rs=recommendationScore(topPriority);
