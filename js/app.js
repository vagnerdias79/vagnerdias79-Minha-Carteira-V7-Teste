...
function loadQuoteCache(){try{var q=JSON.parse(localStorage.getItem(QUOTE_CACHE_KEY)||'{}');if(q&&q.precos)precos=q.precos;if(q&&Number(q.usdBrl)>0)usdBrl=Number(q.usdBrl);if(q&&q.camUpd)camUpd=q.camUpd;}catch(e){}}
...
async function fetchCambio() {
  var providers=[
    async function(){var r=await fetch('https://economia.awesomeapi.com.br/json/last/USD-BRL',{cache:'no-store'});var d=await r.json();return d&&d.USDBRL&&Number(d.USDBRL.bid);},
    async function(){var r=await fetch('https://api.frankfurter.app/latest?from=USD&to=BRL',{cache:'no-store'});var d=await r.json();return d&&d.rates&&Number(d.rates.BRL);},
    ...
    async function(){var r=await fetch('https://brapi.dev/api/v2/currency?currency=USD-BRL&token='+TOK,{cache:'no-store'});var d=await r.json();return d&&d.currency&&d.currency[0]&&Number(d.currency[0].bidPrice);}
  ];
  for(var i=0;i<providers.length;i++){
    try{var bid=await Promise.race([providers[i](),new Promise(function(_,rej){setTimeout(function(){rej(new Error('timeout'));},6500);})]);
      if(bid&&bid>3&&bid<10){usdBrl=bid;camUpd=new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit',second:'2-digit'});saveQuoteCache();return true;}
    }catch(e){}
  }
  return false;
}
...
function renderAll() {
  ...
  text('mpat',fR(t.pat));text('mpatUsd',fU(t.pat/usdBrl));
  htmlSet('mvd','<span class="'+fC(t.vd)+'">'+fR(t.vd)+' no dia ('+fP(t.vdp)+')</span>');
  text('minv',fR(t.custo));text('minvUsd',fU(t.custo/usdBrl));
  htmlSet('mrp','<span class="'+fC(t.res)+'">'+fP(t.rp)+' sobre o custo</span>');
  text('mres',fR(t.res));classSet('mres','val '+fC(t.res));text('mresUsd',fU(t.res/usdBrl));
  htmlSet('mvd2','<span class="'+fC(t.vd)+'">'+fP(t.vdp)+' no dia</span>');
  text('mdiv',fR(divResumo.brl));text('mdivUsd',fU(divResumo.usd));
  text('mret',fP(t.rp));classSet('mret','val '+fC(t.rp));text('mcam','R$ '+usdBrl.toFixed(4));
