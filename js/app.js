...
function buildRebalance(){
  var tbody=document.getElementById('tbReb'); if(!tbody)return; var aporte=Math.max(num('rbAporte'),0),scope=document.getElementById('rbEscopo').value;
  var ativos=carteira.filter(function(a){return (a.pesoAlvo||0)>0&&(scope==='ALL'||a.mercado==='US');}); var total=ativos.reduce(function(s,a){return s+a.qtd*pBRL(a);},0),metaSum=ativos.reduce(function(s,a){return s+(a.pesoAlvo||0);},0); if(!ativos.length||!total||!metaSum){tbody.innerHTML='<tr><td colspan="10" class="empty">Cadastre metas e posições para calcular o rebalanceamento.</td></tr>';return;}
  var finalTotal=total+aporte; var rows=ativos.map(function(a){var atualV=a.qtd*pBRL(a),meta=(a.pesoAlvo||0)/metaSum,atualPct=atualV/total*100,metaPct=meta*100,def=Math.max(finalTotal*meta-atualV,0);return {a:a,v:atualV,at:atualPct,mt:metaPct,des:atualPct-metaPct,def:def};}); var somaDef=rows.reduce(function(s,r){return s+r.def;},0); rows.forEach(function(r){r.buy=somaDef?aporte*r.def/somaDef:0;r.q=r.buy/(pBRL(r.a)||1);}); rows.sort(function(a,b){return b.buy-a.buy||a.des-b.des;});
  var buys=rows.filter(function(r){return r.buy>0.01;}); document.getElementById('rbTotal').textContent=fR(aporte);document.getElementById('rbQtd').textContent=String(buys.length);document.getElementById('rbPrior').textContent=buys.length?buys[0].a.ticker:'Nenhum';
  tbody.innerHTML=rows.map(function(r,i){var action=r.buy>0.01?'<span class="reb-buy">COMPRAR</span>':'<span class="reb-hold">PAUSAR</span>';return '<tr><td>'+(r.buy>0.01?i+1:'—')+'</td><td><span class="tk">'+esc(r.a.ticker)+'</span><div style="font-size:10px;color:var(--text2)">'+esc(r.a.nome)+'</div></td><td>'+esc(r.a.tipo.toUpperCase())+'</td><td>'+fR(r.v)+'</td><td>'+r.at.toFixed(2)+'%</td><td>'+r.mt.toFixed(2)+'%</td><td class="'+fC(r.des)+'">'+fP(r.des)+'</td><td class="reb-buy">'+fR(r.buy)+'</td><td>'+r.q.toFixed(6)+'</td><td>'+action+'</td></tr>';}).join('');
}
