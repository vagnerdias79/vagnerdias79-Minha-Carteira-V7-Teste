...
function loadQuoteCache(){
  try{
    ...
    if(!fresh&&q&&q.usdBrl){try{localStorage.removeItem(QUOTE_CACHE_KEY);}catch(_e){}}
  }catch(e){}
}
...
function nc15BuildGeo(){
  var pat=totais().pat||1;
  var regions={Brasil:0,'América do Norte':0,Europa:0,Asia:0,'Global / Outros':0};
  var mapTicker={
    'INDA':'Asia','VXUS':'Global / Outros','RACE':'Europa','NVO':'Europa','ASML':'Europa','SAP':'Europa',
    'MELI':'América do Norte','VOO':'América do Norte','QQQM':'América do Norte','SCHD':'América do Norte',
    'SHLD':'América do Norte','URA':'Global / Outros','BRK.B':'América do Norte','BRK-B':'América do Norte'
  };
  carteira.forEach(function(a){
    var v=rbValue(a);if(!v)return;
    var tk=String(a.ticker||'').toUpperCase(),r;
    if(a.mercado!=='US')r='Brasil';
    else if(mapTicker[tk])r=mapTicker[tk];
    else if(/india|asia/i.test(String(a.setor||'')))r='Asia';
    else if(/europe|europa/i.test(String(a.setor||'')))r='Europa';
    else r='América do Norte';
    regions[r]+=v;
  });
  var palette={'Brasil':'#22c779','América do Norte':'#2479ff','Europa':'#48d08a','Asia':'#f3bd22','Global / Outros':'#7289a3'};
  var values=Object.keys(regions).map(function(name){return {name:name,value:regions[name],pct:regions[name]/pat*100,color:palette[name]};}).filter(function(x){return x.value>.01;}).sort(function(a,b){return b.value-a.value;});
  ...
  if(map){
    var us=(regions['América do Norte']/pat),eu=(regions.Europa/pat),asia=(regions.Asia/pat),br=(regions.Brasil/pat),other=(regions['Global / Outros']/pat);
    map.style.setProperty('--north-opacity',Math.max(.18,us).toFixed(2));
    map.style.setProperty('--br-opacity',Math.max(.18,br).toFixed(2));
    map.style.setProperty('--eu-opacity',Math.max(.18,eu).toFixed(2));
    map.style.setProperty('--asia-opacity',Math.max(.18,asia).toFixed(2));
    map.style.setProperty('--other-opacity',Math.max(.18,other).toFixed(2));
